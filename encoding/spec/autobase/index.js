...
const encoding13 = {
  preencode (state, m) {
    ...
    if (version >= 2 && m.trace) encoding13_4.preencode(state, m.trace)
  },
  encode (state, m) {
    const flags =
      (m.checkpoint ? 1 : 0) |
    ...
    if (version >= 2 && m.trace) encoding13_4.encode(state, m.trace)
  },
  decode (state, version) {
    if (version === undefined) version = c.uint.decode(state)
    ...
    return {
      ...
      trace: (version >= 2 && (flags & 8) !== 0) ? encoding13_4.decode(state) : null
    }
  }
}
...
const encoding23_1 = c.array(c.uint)

// @autobase/trace
const encoding23 = {
  preencode (state, m) {
    c.uint.preencode(state, m.view)
    encoding23_1.preencode(state, m.blocks)
  },
  encode (state, m) {
    c.uint.encode(state, m.view)
    encoding23_1.encode(state, m.blocks)
  },
  decode (state) {
    const r0 = c.uint.decode(state)
    const r1 = encoding23_1.decode(state)
...
// @autobase/oplog-message-v2.trace, deferred due to recusive use
const encoding13_4 = c.array(encoding23)
...
function getEncoding (name) {
  switch (name) {
    ...
    case '@autobase/trace': return encoding23
