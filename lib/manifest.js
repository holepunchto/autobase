...
const ByteArray = c.array(c.buffer)
...
const Manifest = {
  ...
  inflate (corestore, m, token) {
    if (Buffer.isBuffer(m)) m = c.decode(ByteArray, m)
    return m.map(u => User.inflate(corestore, u, token))
  },
  deflate (m) {
    const deflated = m.map(u => User.deflate(u))
    return c.encode(ByteArray, deflated)
  }
}
...
function inflateCore (corestore, core, token) {
  console.log('inflating core:', core)
