...
class Update {
  ...
  _execute () {
    if (!this.applied) return this._executeLocal()
    return this._executeRemote()
  }
...
class BranchSnapshot {
  ...
  _getAppliedBranch (purging) {
    ...
    return new AppliedBranch(this.autobase, this.nodes, this.length, this.lastUpdate.clock)
  }
  ...
  async _rebuild (clock) {
    ...
    if (hasPurged(lastClock, clock) && this.autobase.localOutput) {
    ...
    if (!this.autobase.localOutput) {
      if (this.lastUpdate) {
        ...
        applied = new AppliedBranch(this.autobase, [], 0, new Map())
      }
    }
  ...
  async _commitUpdate (status, purged) {
    ...
    try {
      await this._applyPending(status.pending)
    } catch (err) {
      safetyCatch(err)
      throw err
    } finally {
    ...
    if (this.autobase.localOutput) {
      ...
      if (status.truncated && (purged === 0)) {
        await localOutput.truncate(localOutput.length - status.truncated)
      }
