  ...
  async _findIntersection (snapshot, clock, operations) {
    ...
    while (node) {
      ...
      if (node.operations < operations) {
        seq += (operations - node.operations) + node.batch[1]
      } else {
        seq -= (node.operations - operations) + node.batch[0]
      }
...
class Update {
  constructor (autobase, clock, lastOutputs, latestOutputs, applied, localOutput) {
    this.autobase = autobase
    this.clock = clock
...
class BranchSnapshot {
  ...
  async _rebuild (clock) {
    ...
    this.lastOutputs = new OutputsBranch(localOutput ? [localOutput] : this.autobase.outputs)
    this.lastUpdate = new Update(this.autobase, clock, lastOutputs, this.lastOutputs, applied, localOutput)
...
class LinearizedCore extends EventEmitter {
  ...
  session (opts = {}) {
    ...
    if (opts.snapshot) {
      ...
      const branch = this._snapshot.copy({ writable: false })
