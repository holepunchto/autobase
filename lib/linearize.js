...
class OutputsBranch {
  ...
  async fastForward (clock) {
    ...
    for (const [snapshot, head] of this.heads) {
      console.log('FAST FORWARD CHECK:', clock, head.clock)
      if (!lte(clock, head.clock)) continue
      console.log('FFING')
      this.operations = head.operations
      this.length = snapshot.length
      this.snapshot = snapshot
      return { length: this.length, operations: this.operations }
    }
    return null
  }
...
class Update {
  ...
  async _findAncestors () {
    ...
    if (p2 && !p1) {
      ...
      console.log('P2 && !P1 HEAD:', this.applied.head)
      while (this.applied.head) {
        console.log('this.applied.head:', this.applied.head)
  ...
  async _execute () {
    ...
    console.log('P1:', p1, 'P2:', p2, 'PENDING:', pending.length)

    this.p1 = p1
    this.p2 = p2
    this.nodes = (p1.length > p2.length) ? this.applied.slice() : []

    console.log('after slicing, this.nodes.length:', this.nodes.length, 'values:', this.nodes.map(n => n.value.toString()))
    ...
    console.log('this.appended:', this.appended, 'this.truncated:', this.truncated)
...
class Snapshot extends EventEmitter {
  constructor (autobase, clock, opts) {
    super()
    this.autobase = autobase
    this.clock = clock
  ...
  decrement () {
    if (--this.references > 0 || !this.lastUpdate) return Promise.resolve()
  ...
  get (seq, opts = {}) {
    if (!this.lastUpdate) return null
    console.log('getting seq:', seq, 'length:', this.lastUpdate.length)
...
module.exports = class LinearizedCore extends EventEmitter {
  constructor (autobase, opts = {}) {
    ...
    if (!this._snapshot) {
      this._snapshot = new Snapshot(this.autobase, snapshotKey, opts)
  ...
  close () {
    ...
    return this._snapshot.decrement()
  }
}
