...
class AppliedBranch {
  constructor (autobase, oldLength, nodes) {
...
class OutputsBranch {
  ...
  async _open () {
    ...
    const badSnapshots = []
    console.log('snapshot lengths:', this.snapshots.map(s => s.length))
...
class Update {
  ...
  async _applyPending (pending) {
    ...
    while (pending.length) {
      ...
      const appliedStart = this.nodes.length

      try {
        await this.applyFunction(this.view, batch, clocks, node.change)
      } catch (err) {
        while (this.nodes.length > appliedStart) {
          this.nodes.pop()
        }
        throw err
      }
      if (this.nodes.length === appliedStart) {
  ...
  async _execute () {
    ...
    console.log('OLD LENGTH:', oldLength, 'P1:', p1)
    ...
    try {
      ...
      if (this.autobase.localOutput) {
        // If we're using a local output, then the old branch will always track the outputs
        // P1 will never be located (the applied branch is always empty).
        this.appended = this.nodes.length
        this.truncated = this.autobase.localOutput.length - p2.length
      } else {
        this.appended = this.length - p1.length
...
class Snapshot extends EventEmitter {
  ...
  async _rebuild (clock) {
    ...
    const lastLength = this.lastUpdate ? this.lastUpdate.length : 0

    const nodes = !this.autobase.localOutput ? this.nodes : []
    const applied = new AppliedBranch(this.autobase, lastLength, nodes)
    ...
    if (this.autobase.localOutput) {
      ...
      // lastUpdate.latestOutputs.length = localOutput.length
    }
