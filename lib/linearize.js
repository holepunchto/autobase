...
class AppliedBranch {
  constructor (autobase, baseLength, nodes, { truncated, head } = {}) {
    this.autobase = autobase
    this.nodes = nodes
    this.baseLength = baseLength
    this.truncated = opts.truncated || 0
    this.head = opts.head || null
    this.opened = !!opts.head
  ...
  checkout () {
    return new AppliedBranch(this.autobase, this.baseLength, this.nodes, {
...
class Update {
  constructor (autobase, clock, outputsBranch, appliedBranch) {
    this.autobase = autobase
    this.clock = clock
    this.outputsBranch = outputsBranch
    this.appliedBranch = appliedBranch
    this.appended = 0
    this.truncated = 0
    this.nodes = null
    this._applying = null
  ...
  get length () {
    return this.outputsBranch.length + this.nodes.length
  }
  ...
  async execute ({ view, apply }) {
    ...
    p2 = await this.outputsBranch.fastForward(this.clock)
    ...
    if (!p2) {
      for await (const node of this.autobase.createCausalStream({ clock: this.clock })) {
        if (!p1) p1 = await this.appliedBranch.update(node)
        if (!p2) p2 = await this.outputsBranch.update(node)
        if (p2 || p1) break
        pending.push(node)
      }
    }
    ...
    if (p2 && !p1) {
      const branch = this.appliedBranch.checkout()
      while (branch.head) {
        p1 = await this.outputBranch.intersect(branch.head)
        if (p1) break
        await branch.pop()
      }
    }

    // TODO: If p1 && !p2, potentially roll the stream ~5 back because the output is probably there

    this.nodes = [...this.appliedBranch.slice()]
    await this._applyPending(pending, view, apply) // This will push directly into this.nodes

    if (this.appliedView.operations <= this.outputsView.operations) {
      // If P1 <= P2, the truncation size is the distance from the old head to P1 (the amount truncated in the appliedBranch)
      // The appended size is the distance from P1 to P2 + the (applied) pending size
      this.truncated += this.appliedView.truncated
    }
  }

  get (seq, opts) {
    if (seq < 0 || seq > this.length - 1) return null
    if (seq < this.outputsBranch.length - 1) return this.outputsBranch.get(seq)
    return this.nodes[seq - this.outputsBranch.length]
  }
