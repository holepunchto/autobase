...
class Linearizer {
  ...
  async _update () {
    ...
    console.log('LATEST CLOCK:', latestClock)
    if (this.invalidated) return null

    if (!this._keyCompressor.initialized) {
      console.log('INITIALIZING COMPRESSOR AT LENGTH:', this.outputLength)
    ...
    for await (const node of this.autobase.createCausalStream()) {
      ...
      console.log('NODE:', node, 'HEAD:', head, 'EQ?', head && node.eq(head), 'LT?', head && head.lt(node), 'CONTAINS?', head && head.contains(node), 'ID:', head && head.id)
      if (this.invalidated) return null
...
module.exports = class LinearizedView extends EventEmitter {
  constructor (autobase, opts = {}) {
    ...
    this[promises] = true
    this.autobase = autobase
    this.writable = !!autobase.localOutput
