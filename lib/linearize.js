...
class Update {
  constructor (autobase, clock, view, apply, applied, lastOutputs, latestOutputs) {
    this.autobase = autobase
    this.clock = clock
    this.view = view
...
module.exports = class LinearizedCore extends EventEmitter {
  ...
  async update () {
    const applied = new AppliedBranch(this.autobase, this._lastUpdate ? this._lastUpdate.nodes : [])
    const latestOutputs = new OutputsBranch(this.autobase.outputs)
    const lastOutputs = this._lastUpdate ? this._lastUpdate.latestOutputs : null

    const clock = this._snapshot || await this.autobase.latest()
    this._lastUpdate = new Update(this.autobase, clock, this.view, this._apply, applied, lastOutputs, latestOutputs)
    await this._lastUpdate.execute()

    const appended = this._lastUpdate.appended
    const truncated = this._lastUpdate.truncated

    if (this.autobase.localOutput && this._lastUpdate.nodes.length) {
      const localOutput = this.autobase.localOutput
      const lastUpdate = this._lastUpdate
      if (localOutput.length === 0) {
        lastUpdate.nodes[0].header = this._header
      }
      if (truncated) {
        await localOutput.truncate(localOutput.length - truncated)
      }
      await localOutput.append(lastUpdate.nodes.map(n => OutputNode.encode(n)))
    }

    if (truncated) this.emit('truncate', this._lastUpdate.length - truncated)
  ...
  get status () {
    ...
    return {
      appended: this._lastUpdate.appended,
      truncated: this._lastUpdate.truncated
    }
  }

  async get (seq, opts = {}) {
    if (!this._lastUpdate) return null
    let block = await this._lastUpdate.get(seq, opts)
  ...
  append (blocks) {
    if (!this._pendingUpdate) throw new Error('Can only append to a LinearizedCore inside of an apply function')
    return this._pendingUpdate.append(blocks)
  }
  ...
  snapshot () {
    if (!this._lastUpdate) throw new Error('Can only snapshot after an update')
    return this._copy({ clock: this._lastUpdate.clock })
  }
