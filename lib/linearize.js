...
const { eq, hasPurged } = require('./clock')
...
class AppliedBranch {
  constructor (autobase, nodes, oldLength, oldClock) {
    ...
    this.nodes = nodes
    console.log('CREATING BRANCH WITH OLD LENGTH:', oldLength)
...
class Update {
  ...
  async _execute () {
    ...
    console.log('p1:', p1, 'p2:', p2, 'oldLength:', oldLength, 'pending:', pending.length)

    const nodes = (p1.length > p2.length) ? this.applied.slice() : []
    const length = this.latestOutputs.length + nodes.length + pending.length
...
class BranchSnapshot {
  ...
  async open (view, clock) {
    if (this.lastUpdate) return
    this.clock = clock
    this.view = view
    return this._rebuild(this.clock)
  }
  ...
  _getAppliedBranch (purging) {
    ...
    if (purging) return new AppliedBranch(this.autobase, [], this.length, new Map())
    const nodes = !this.autobase.localOutput ? this.nodes : []
    return new AppliedBranch(this.autobase, nodes, this.length, this.lastUpdate.clock)
  }
  ...
  async _rebuild (clock) {
    ...
    this.lastUpdate = new Update(this.autobase, clock, applied, lastOutputs, outputs)
    this.lastOutputs = outputs

    const updateInfo = await this.lastUpdate.execute()
    await this._commitUpdate(updateInfo, purged)

    return this.appended > 0
  }

  async _commitUpdate (status, purged) {
    const { pending, nodes, truncated, length } = status
    console.log('length:', length, 'truncated:', truncated, 'pending length:', pending.length, 'nodes length:', nodes.length)
    this.status = status
    this.nodes = nodes
    this.length = length

    if (!this.view) {
      this.view = this.viewFunction ? this.viewFunction(this.session()) : this.session()
    }
    ...
    for (const session of this._sessions) {
      if (this.truncated) session.emit('truncate', this.lastUpdate.length - truncated)
      if (this.appended) session.emit('append')
    }
  }
...
class LinearizedCore extends EventEmitter {
  ...
  async _open () {
    ...
    if (!this._snapshotClock) this._snapshotClock = await this.autobase._getLatestClock()
    this.opened = true
  }
  ...
  unwrap () {
    return this._snapshot.session({ ...this.opts, unwrap: true })
  }

  wrap () {
    return this._snapshot.session({ ...this.opts, unwrap: false })
  }
  ...
  session (opts = {}) {
    if (!opts.snapshot && !opts.checkout) return this._snapshot.session(opts)
    if (opts.snapshot) {
