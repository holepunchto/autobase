...
module.exports = class Linearizer {
  ...
  addHead (node) {
    ...
    this.updated = true
    if (node.writer.isIndexer) this.indexersUpdated = true
  ...
  _next (removed) {
    ...
    for (const head of this.heads) {
      ...
      if (node && !tieBreak(head, node)) continue
      node = head
    }
    ...
    while (true) {
      ...
      for (const dep of node.dependencies) {
        ...
        if (best && !tieBreak(dep, best)) continue
        best = dep
      }
  ...
  _shift () {
    if (!this.indexersUpdated) return []
    ...
    while (node.dependencies.size) {
      ...
      if (!next) {
        for (const t of this._tails(node)) {
          if (next && !tieBreak(t, next)) continue
          next = t
        }
      }
