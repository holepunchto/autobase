...
module.exports = class Linearizer {
  constructor (indexers, heads = [], removed = []) {
    ...
    for (const head of this.heads) {
      this.headsIndexed.add(head)
    }
    for (const { writer, length } of removed) {
      this.removed.set(writer, length)
    }
  ...
  shouldAck (writer) {
    const prev = writer.length && writer.getCached(writer.length - 1)

    // check if we can merge branches
    let merged = !!prev

    for (const tail of this.tails) {
      if (merged) merged = prev.clock.includes(tail.writer, tail.length)

      if (tail.value === null) continue
      if (prev && !this._strictlyNewer(tail, prev)) continue
      if (this._shouldAckNode(tail, writer)) return true
    }

    for (const merge of this.merges) {
      merged &&= prev.clock.includes(merge.writer, merge.length)

      if (prev && !this._strictlyNewer(merge, prev)) continue
      if (this._shouldAckNode(merge, writer)) return true
    }
  ...
  _shouldAckNode (target, writer) {
    ...
    while (stack.length) {
      ...
      if (seen.size >= thres) {
        ...
        // target is already confirmed
        if (confs.size >= thres) return false
      }
      ...
      if (seen.size >= thres || !seen.has(writer)) return true
    }
