...
module.exports = class Linearizer {
  ...
  _strictlyNewer (object, parent) {
    ...
    // parent does not see object
    if (!parent.clock.includes(object.writer, object.length)) return false

    // whole subdag of parent must see object
    for (const d of parent.dependencies) {
      if (!this._strictlyNewer(object, d)) return false
    }
  ...
  _isConfirmed (target, parent = null) {
    ...
    function confirms (node, seen, confs) {
      ...
      for (const dep of node.dependents) {
        ...
        if (last && confirms(last, new Set(seen), next)) {
          return true
        }
        last = dep
      }

      if (last && confirms(last, seen, confs)) {
        return true
      }
    ...
    function preferred (confs) {
      ...
      for (const w of idx) {
        ...
        // writer's prev node in parent clock
        const prev = parent.clock.get(w)

        // writer is only available if:
        //   1. prev is yielded
        //   2. prev is seen by target

        // any writer otherwise avaialble would be in confs already

        const isYielded = removed.get(w) < prev
        const isSeen = target.clock.get(w) === prev

        if (isYielded || isSeen) available++
  ...
  _removeBatch (node) {
    ...
    while (node.batch !== 1) { // its a batch!
      if (!node.dependents.size) return false
