...
class Node {
  ...
  active () {
    ...
    this.ordering = this.dependencies.size
...
module.exports = class Linearizer {
  ...
  update () {
    ...
    const batch = this._yield(indexed)
    batch.forEach(n => this._removeNode(n))

    return this.tip.flush(batch)
  }
  ...
  _yield (batches) {
    ...
    while (batches.length) {
      ...
      while (nodes.length) {
        ...
        if (!writers.has(node.writer)) {
          const batch = { start: node.writer.indexed, end: node.length }
          writers.set(node.writer, batch)
        }

        const batch = writers.get(node.writer)
        if (node.length > batch.end) batch.end = node.length

        nodes.push(...node.dependencies)
      }

      const topo = new Topolist()
      const yielded = new Set()

      for (const [writer, batch] of writers) {
        while (batch.start < batch.end) {
          const node = writer.get(batch.start++)
          yielded.add(node)

          if (node.ordering === 0) tails.push(node)
        }
      }
      ...
      while (tails.length) {
        ...
        for (const dep of tail.dependents) {
          if (--dep.ordering === 0 && yielded.has(dep)) tails.push(dep)
        }
      }

      result.push(...topo.tip)
    }
