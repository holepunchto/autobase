...
class Node {
  ...
  active () {
    ...
    if (this.writer._isActiveIndexer) this.clock.set(this.writer.core.key, this.length)
  }
...
module.exports = class Linearizer {
  ...
  addHead (node) {
    ...
    this.tip.add(node)
    if (node.writer._isActiveIndexer) this.consensus.addHead(node)
  ...
  shouldAck (writer, pending = false) {
    if (!writer || !writer._isActiveIndexer) return false
    ...
    for (const head of this.heads) {
      if (!head.writer._isActiveIndexer) return true
