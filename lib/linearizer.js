...
module.exports = class Linearizer {
  ...
  shouldAck (writer, pending = false) {
    ...
    return this._shouldMergeHeads(writer)
  }
  ...
  // merge if there is any head can see more acks over a value
  _shouldMergeHeads (writer) {
    if (this.heads.size === 1) return false

    const prev = writer.head()

    // merge any head above an unyielded value
    for (const head of this.heads) {
  ...
  _acksFromNode (target, view) {
    ...
    for (const idx of this.indexers) {
      ...
      if (next >= idx.indexed || target.clock.includes(idx.core.key, next)) continue
