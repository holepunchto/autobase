...
module.exports = class Linearizer {
  constructor (indexers, { heads = [], writers = new Map() } = {}) {
    ...
    this.dag = new Consensus(indexers)
    this._initialHeads = heads.slice(0)
    this._strictlyAdded = null

    for (const { key, length } of heads) {
      this.dag.removed.set(key, length)
    }
  }

  get indexers () {
    return this.dag.indexers
  }
  ...
  addHead (node) {
    ...
    if (node.writer.isIndexer) this.dag.addHead(node)
  ...
  shouldAck (writer, pending = false) {
    ...
    for (const tail of this.tails) {
      if (this.dag._shouldAckNode(tail, writer)) return true
    }
  ...
  _shouldAckHeads (writer) {
    ...
    for (const head of this.heads) {
      ...
      while (stack.length) {
        ...
        if (node.value !== null) {
          const acks = this.dag._acksFromNode(node, head)
          const prevAcks = this.dag._acksFromNode(node, prev)
  ...
  _removeNode (node) {
    ...
    this.dag.merges.delete(node)
    this.heads.delete(node)

    // update the removed clock
    if (node.writer.isIndexer) {
      this.dag.removed.set(node.writer.core.key, node.length)
    }
