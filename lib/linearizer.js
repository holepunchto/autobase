...
class Node {
  ...
  updateCache (index) {
    if (this.cacheIndex === index) return
    this.newer.clear()
...
module.exports = class Linearizer {
  constructor (indexers, { heads = [], removed = [], cacheIndex = 0 } = {}) {
    ...
    for (const { writer, length } of removed) {
  ...
  _strictlyNewer (object, parent) {
    ...
    if (parent.cacheIndex === this.cacheIndex && parent.newer.has(object)) {
      return true
    }
    ...
    while (stack.length) {
      ...
      node.updateCache(this.cacheIndex)
      ...
      if (!parent.clock.includes(node.writer.core.key, node.length)) continue

      const isSeen = object.clock.includes(node.writer.core.key, node.length)
      const isNewer = node.clock.includes(object.writer.core.key, object.length)

      if (!isSeen && !isNewer) return false

      if (isNewer) newer.add(object)
    ...
    parent.cacheIndex = this.cacheIndex

    for (const node of newer) {
      parent.newer.add(node)
      node.newer.add(object)
    }
  ...
  _isConfirmed (target, parent = null) {
    ...
    while (stack.length) {
      ...
      if (parent && !parent.clock.includes(node.writer.core.key, node.length)) return false

      if (idx.includes(node.writer)) {
    ...
    function preferred (confs) {
      ...
      for (const w of idx) {
        ...
        const prev = parent.clock.get(w.core.key)

        // writer available if prev is yielded
        const isYielded = removed.includes(w.core.key, prev)

        // writer available if prev is seen by target
        const isSeen = target.clock.includes(w.core.key, prev)
