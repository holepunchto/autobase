...
module.exports = class Linearizer {
  ...
  addHead (node) {
    ...
    this.tip.add(node)
    if (node.writer.isIndexer) this._pendingConsensusHeads.push(node)
  ...
  update () {
    ...
    // apply consensus heads jit to avoid batch sizes affecting it
    for (let i = 0; i < this._pendingConsensusHeads.length; i++) {
      const head = this._pendingConsensusHeads[i]
      this.consensus.addHead(head)
      while (true) {
    ...
    if (this._pendingConsensusHeads.length > 0) {
      this._pendingConsensusHeads = []
    }
