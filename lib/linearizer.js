...
module.exports = class Linearizer {
  ...
  shouldAck (writer) {
    ...
    let allChecked = writer.core.writable // only relevant if checking the local writer
    ...
    for (const w of this.indexers) {
      ...
      if (w.ackCheckedAt !== w.available) allChecked = false
    }
    ...
    if (writer.core.writable) {
      for (const w of this.indexers) w.ackCheckedAt = w.available
    }
