...
module.exports = class Linearizer {
  ...
  addHead (node) {
    ...
    for (let i = 0; i < node.dependencies.length; i++) {
      ...
      if (dep.clock !== null) dep.dependents.push(node)
  ...
  _isTail (node) {
    for (let i = 0; i < node.dependencies.length; i++) {
      if (node.dependencies[i].clock !== null) return false
    }
  ...
  _next (tails) {
    ...
    function votesFor (node) {
      ...
      for (const [writer, length] of node.clock) {
        ...
        if (dep === null || dep.clock === null) continue
  ...
  _shift () {
    ...
    for (let i = 0; i < this.headsIndexed.length; i++) {
      ...
      if (result.node.clock.get(head.writer) >= head.length) {
        if (popAndSwap(this.headsIndexed, i)) i--
      }
    }

    this.headsIndexed.push(result.node)
