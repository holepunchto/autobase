...
module.exports = class SystemView extends ReadyResource {
  ...
  flushLength () {
    let uniq = this.core.length ? 0 : 1 // account for header
    for (const { key } of this._changes) {
      if (!this._clockUpdates.has(b4a.toString(key, 'hex'))) uniq++
    }
    return this.core.length + uniq + this._clockUpdates.size + 1
  }
...
function generateEntropy (indexers, length, entropy) {
  const buffer = b4a.alloc(indexers.length * 32 + 8 + 32)
