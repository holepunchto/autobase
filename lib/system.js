...
module.exports = class SystemView extends ReadyResource {
  ...
  async flush(views, encryptionLength) {
  ...
  setVersion(version) {
    if (version < this.version) {
      throw new Error('System version can only increase')
    }
  ...
  static async members(core, { onlyActive = false } = {}) {
    const members = []
    const sys = new SystemView(core)
    for await (const member of sys.list()) {
      if (onlyActive && member.value.isRemoved) continue
      members.push(member)
    }
    await sys.close()
    return members
