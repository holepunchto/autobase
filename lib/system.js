...
const [GENESIS_ENTROPY, NS_ENTROPY] = crypto.namespace(`autobase/entropy/v${AUTOBASE_VERSION}`, 2)
...
module.exports = class SystemView extends ReadyResource {
  ...
  static genesisEntropy = GENESIS_ENTROPY
  ...
  async getIndexedInfo (length = this.core.signedLength) {
    ...
    if (length === this.core.length) {
      return { version: this.version, members: this.members, pendingIndexers: this.pendingIndexers, indexers: this.indexers, heads: this.heads, views: this.views, entropy: this.entropy }
    }

    const node = length === 0 ? null : await this.db.getBySeq(length - 1)
    if (node === null) return { version: 0, members: 0, pendingIndexers: [], indexers: [], heads: [], views: [], entropy: GENESIS_ENTROPY }
  ...
  async _reset (info) {
    ...
    this.entropy = info === null ? GENESIS_ENTROPY : info.value.entropy
  ...
  async flush (views) {
    ...
    if (this.indexerUpdate) this._refreshEntropy()
  ...
  getEntropy (indexers) {
    return generateEntropy(indexers, this.entropy)
  }

  _refreshEntropy () {
    this.entropy = this.getEntropy(this.indexers)
  }
  ...
  async fork (indexers, manifests, store) {
    this.indexers = []
    this.pendingIndexers = []
    ...
    this._refreshEntropy()

    for (const view of this.views) {
      const ref = await store.findViewByKey(view.key)

      const prologue = await ref.getPrologue(view.length)
      const key = await store.createView(manifests, ref.name, prologue, this.entropy)
    ...
    await this._batch.ready()

    for (const { type, key, value } of blocks) {
      if (type !== 'put') throw new Error('NOT A PUT')
...
function generateEntropy (indexers, entropy) {
  const buffer = b4a.alloc((indexers.length + 1) * 32)
  const state = { start: 0, end: buffer.byteLength, buffer }

  for (const { key } of indexers) c.fixed32.encode(state, key)
  c.fixed32.encode(state, entropy)
