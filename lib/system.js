...
module.exports = class SystemView extends ReadyResource {
  ...
  _onundo (removed) {
    while (removed-- > 0) {
      ...
      this.base._onsystemremovewriter(add)
    }
  }

  hasWriter (key) {
    for (const w of this.digest.writers) {
      if (w.key === b4a.toString(key, 'hex')) return true
    }
  ...
  async flush (changes, user, writers, heads) {
    ...
    for (let i = 0; i < changes; i++) {
      ...
      if (c.add) {
        const value = {}
        await batch.put(b4a.toString(c.add, 'hex'), value, { keyEncoding: this.subs.writers })

        this.digest.writers.push({
          key: b4a.toString(c.add, 'hex'),
          length: 0,
          pending: false
        })
      }
