...
module.exports = class SystemView extends ReadyResource {
  constructor (core) {
    ...
    this.digest = { heads: [], writers: [], indexes: ['system'] }
  }
  ...
  async flush (update, node, writers) {
    ...
    for (const c of this._updating) {
      ...
      if (!restart && c.indexer) restart = true
    }
    ...
    for (const w of writers) {
      for (const idx of digest.writers) {
        if (b4a.equals(idx.key, w.core.key)) {
          const seen = node.clock.get(w.core.key)
