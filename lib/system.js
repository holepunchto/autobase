...
module.exports = class SystemView extends ReadyResource {
  ...
  async flush (update) {
    ...
    this._clockUpdates.clear()
    this._updateView('system', this.core.length + this._batch.length + 1, 0) // plus 1 due to info itself

    for (const { core } of update.views) {
      if (core.name === 'system') continue // updated above...
      core.index = this._updateView(core.name, core.length, core.index)
    }
