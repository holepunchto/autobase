...
module.exports = class SystemView extends ReadyResource {
  ...
  async flush (views) {
    ...
    for (const [hex, length] of this._clockUpdates) {
      ...
      assert(info, 'Clock updates should be linked')

      const value = { isIndexer, isRemoved: info.isRemoved, length }
  ...
  async fork (indexers, views) {
    ...
    // truncate needs to respect batch
