...
module.exports = class SystemView extends ReadyResource {
  constructor (core, checkout = 0) {
    ...
    this.core = core
    this.db = new Hyperbee(core, { keyEncoding: 'binary', extension: false, checkout })
  ...
  async getIndexedInfo (length = this.core.indexedLength) {
    ...
    const co = this.db.checkout(length)

    try {
      const info = await co.get('info', { valueEncoding: Info, keyEncoding: DIGEST })
      if (info === null) return { version: 0, members: { active: 0, total: 0 }, pendingIndexers: [], indexers: [], heads: [], views: [] }
      return info.value
    } finally {
      await co.close()
    }
  ...
  async add (key, { isIndexer = false, length = 0, isPending = true } = {}) {
