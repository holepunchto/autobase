...
module.exports = class SystemView extends ReadyResource {
  ...
  async deltas (undo) {
    if (undo === 0) return null

    let popped = 0
    let head = null
    let tail = null

    for await (const data of this.db.createHistoryStream({ reverse: true })) {
      if (b4a.equals(data.key, INFO)) {
        if (head === null) {
          head = data
        } else {
          tail = data
    ...
    if (undo === 1) {
      popped++ // the bee header
      tail = null
      undo = 0
    }

    if (undo !== 0) throw new Error('Not enough nodes in system')

    const h = c.decode(Info, head.value).views
    const t = tail === null ? [] : c.decode(Info, tail.value).views

    const views = new Array(1 + h.length)

    views[0] = popped

    for (let i = 1; i < views.length; i++) {
      const j = i - 1
      views[i] = h[j].length - (j < t.length ? t[j].length : 0)
    }
  ...
  async update () {
    ...
    // TODO: either drop this entirely or pass cache bust flag
  ...
  async flush (views) {
    ...
    for (const { view, length, key } of views) {
