...
const MemoryView = require('./memory-view')
...
module.exports = class Rebaser {
  ...
  async commit () {
    if (!this.index.length) {
      await this.index.append(Header.fullEncode({
        protocol: INDEX_TYPE
      }))
    }
    ...
    if (this._init && (this.truncation || this.index.length <= 1)) {
      // All reducers should reduce into a memory view.
      memView = MemoryView.from(this.index)
      await this._init(memView)
    }
