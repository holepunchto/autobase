...
module.exports = class Fork {
  ...
  async _upgrade() {
    ...
    const record = await this.base._getEncryptionRecord()

    this.system = new SystemView(await this._get('_system', this.length))
    this.encryption = new EncryptionView(this.base, await this._get('_encryption', -1), record)
    ...
    await this.encryption.update(payload)
  ...
  async _getRecipients() {
    const core = this.system.core.session({ active: false })
    const members = await SystemView.members(core, { onlyActive: true })
    return this.store.getManifestPublicKeys(members)
  }
