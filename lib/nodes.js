...
function encodeIndex (idx) {
  ...
  const toEncode = { ...idx, key: Buffer.from(idx.key, 'hex'), clock }
  return IndexNodeSchema.fullEncode(toEncode)
}
...
function encodeInput (inp) {
  ...
  if (inp.value && !Buffer.isBuffer(inp.value)) inp.value = Buffer.from(inp.value)
  return InputNodeSchema.fullEncode({ ...inp, links })
}
