...
class ViewStore {
  ...
  get (opts, moreOpts) {
    ...
    let view = this.byName.get(opts.name)

    if (!view) {
      const preload = this._preload(name, cache)
      const core = this.base.store.get({ preload })
      view = new View(name, core)
      this.byName.set(opts.name, view)
    }
    ...
    return view.createSession(this.atom, valueEncoding)
  }
  ...
  async getNextCore (indexers, name, prologue, opts = {}) {
    const manifest = this._createManifest(indexers, name, prologue)
  ...
  async _preload (name, cache) {
    ...
    if (system === null) {
      return this._freshCorePreload(indexerManifests, name, cache)
    }
    ...
    // infer which view
    const v = this.findViewByName(indexerManifests, views, name)
  ...
  _freshCorePreload (indexerManifests, name, cache) {
    return {
  ...
  getViewKey (indexerManifests, prologue, name) {
  ...
  getBootstrapSystemKey (manifest) {
    return Hypercore.key(this._createManifest([manifest], '_system', null))
  }

  async findViewByName (indexerManifests, views, name) {
    if (indexerManifests.length === 0) return null

    const namespace = this._deriveNamespace(name, indexerManifests[0].manifest.signers[0].namespace)
