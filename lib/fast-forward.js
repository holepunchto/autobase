...
const ViewEncryption = require('./encryption.js')
...
module.exports = class FastForward {
  constructor (base, key, { timeout = DEFAULT_OP_TIMEOUT, verified = true, minimum = DEFAULT_MIN_FF, force = false } = {}) {
    ...
    this.force = force
    this.core = base.store.get({ key, active: true })
    this.length = 0
    this.views = []
    this.entropy = null
    this.indexers = []
  ...
  async upgrade () {
    ...
    try {
      ...
      return { length: this.length, force: this.force, key: this.key, indexers: this.indexers, views: this.views, entropy: this.entropy }
    } catch {
      this.failed = true
      return null
    } finally {
  ...
  async _upgrade () {
    ...
    if (this.base.encryptionKey) {
      const encryption = new ViewEncryption(this.base, this.core)
      await encryption.ready()

      await this.core.setEncryption(await encryption.get('_system'))
    }
