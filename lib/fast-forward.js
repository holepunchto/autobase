...
const AutobaseEncryption = require('./encryption')
...
module.exports = class FastForward {
  ...
  async _ensureEncryption () {
    if (this.core.manifest.version === 1) {
      const key = AutobaseEncryption.getBlockKey(this.base.bootstrap, this.base.encryptionKey, '_system')
      this.core.setEncryptionKey(key, { block: true })
      return true
    }
    ...
    const encryptionCore = this.base.store.get({ key: linked[0], active: true })
    await encryptionCore.ready()

    await this._waitForAppend(encryptionCore)

    const encryptionView = new AutobaseEncryption(this.base, encryptionCore)
    const encryption = encryptionView.get('_system')
