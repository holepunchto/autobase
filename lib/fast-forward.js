...
module.exports = class FastForward {
  constructor (base, key, { timeout = DEFAULT_OP_TIMEOUT, verified = true, minimum = DEFAULT_MIN_FF, force = false } = {}) {
    this.base = base
    this.key = key
    console.trace('FAST FORWARD', key)
  ...
  async _ensureEncryption () {
    if (this.core.manifest.version === 1) {
      const key = EncryptionView.getBlockKey(this.base.bootstrap, this.base.encryptionKey, this.key)
    ...
    const encryptionView = new EncryptionView(this.base, encryptionCore)
    const encryption = encryptionView.getViewEncryption('_system')
