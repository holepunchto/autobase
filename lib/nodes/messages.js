...
const Header = {
  preencode (state, req) {
    ...
    if (req.version) {
      c.string.preencode(state, req.version)
    }
  },
  encode (state, req) {
    const s = state.start++
    ...
    if (req.version) {
      flags |= 2
      c.string.encode(state, req.version)
    }
    state.buffer[s] = flags
  },
  decode (state) {
    ...
    return {
      protocol: (flags & 1) !== 0 ? c.string.decode(state) : null,
      version: (flags & 2) !== 0 ? c.string.decode(state) : null
    }
  }
}
...
const ClockWithOperations = {
  preencode (state, req) {
    c.uint.preencode(state, req.id)
    ClockArray.preencode(state, req.clock)
    c.uint.preencode(state, req.operations)
  },
  encode (state, req) {
    c.uint.encode(state, req.id)
  ...
  decode (state) {
    return {
      ...
      operations: c.uint.decode(state)
    }
  }
}
// TODO: Use a bitfield instead of encoding IDs into each array element
const MultipleClocks = c.array(ClockWithOperations)
...
const Node = {
  preencode (state, req) {
    ...
    if (req.clocks) MultipleClocks.preencode(state, req.clocks)
  ...
  encode (state, req) {
    ...
    if (req.clocks) {
      bitfield |= 64
      MultipleClocks.encode(state, req.clocks)
    }
  ...
  decode (state) {
    ...
    return {
      ...
      clocks: (bitfield & 64) !== 0 ? MultipleClocks.decode(state) : null,
