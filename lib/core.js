...
module.exports = class Autocore extends ReadyResource {
  ...
  async flushWriteBatch (batch, treeLength) {
    ...
    if (treeLength < currentLength) this._emitTruncate(treeLength)
    this._emitAppend()
  }
  ...
  _emitTruncate (length) {
    for (const session of this.sessions) {
      if (session.snapshotted === false && session.indexed === false) session.emit('truncate', length)
    }
  }
