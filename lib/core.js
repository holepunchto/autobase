...
class AutocoreSession extends EventEmitter {
  ...
  async get (index, opts) {
    if (this.opened === false) await this.ready()
    if (this.closing === true) throw SESSION_CLOSED()
    if (this._source.core) await this._source.core.ready()
...
module.exports = class Autocore extends ReadyResource {
  ...
  async _open () {
    ...
    if (this.base.encryptionKey && !this.originalCore.encryption) {
      await this.originalCore.setEncryptionKey(this.base.encryptionKey)
    }
    ...
    if (sys) {
      ...
      for (const { key, length } of sys.views) {
        if (this.isKeyOwner(key)) {
          if (!b4a.equals(this.originalCore.key, key)) {
            const old = this.originalCore
            await this.originalCore.close()

            const encryptionKey = this.base.encryptionKey
            this.originalCore = this.base.store.get({ key, encryptionKey, isBlockKey: true, cache: !!old.cache })
            await this.originalCore.ready()
          }

          this.core = this.originalCore.batch({ autoClose: false, session: false, checkout: length })
    ...
    if (!this.core) this.core = this.originalCore.batch({ autoClose: false, session: false, checkout: 0 })
  ...
  async _tryMigrateCore () {
    ...
    if (this.signer.getSignableLength(indexers) < length) return

    for (const idx of indexers) {
      if (!idx.core.manifest) return // only triggered for bootstrap
    }
    ...
    const batch = core.batch({ autoClose: false, session: false, checkout })
    await batch.ready()
