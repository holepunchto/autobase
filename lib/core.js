...
module.exports = class Autocore extends ReadyResource {
  ...
  async _open () {
    ...
    this._initialised = this.originalCore.manifest.signers.length > 0
  ...
  async _initialise () {
    ...
    if (!bootstrap || !bootstrap.core.manifest) return

    if (!(await this.migrateTo([bootstrap], 0))) return
  ...
  async flush () {
    ...
    if (!this._initialised) await this._initialise()
  ...
  async _migrateCurrentCore (next, length, indexers) {
    if (length === 0) {
      ...
      // copy state over
      const additional = []
      const start = this.core.indexedLength
      for (let i = start; i < length; i++) {
        additional.push(await this.core.getRaw(i))
      }

      if (this.core.session.core !== next.core) {
        await next.core.copyFrom(this.core.session.core, null, { start, length, additional })
      }
    }
