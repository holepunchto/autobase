...
module.exports = class Autocore extends ReadyResource {
  ...
  async _open () {
    ...
    if (sys) {
      ...
      for (const v of sys.views) {
        if (b4a.equals(v.key, this.originalCore.key)) {
          this.core = this.originalCore.batch({ autoClose: false, session: false, checkout: v.length })
          break
        }
      }
    }
  ...
  isKeyOwner (key) {
    return this.base._viewStore.getByKey(key) === this
  }
