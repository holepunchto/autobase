...
module.exports = class Autocore extends ReadyResource {
  ...
  async flushWriteBatch (batch, treeLength, atomizer) {
    ...
    await this.core.state.overwrite(batch.state, { treeLength, atomizer })
  }
  ...
  async flush (length, atomizer) {
    ...
    const { valid, promise } = await this._flush(length, atomizer)
    atomizer.exit()
