...
module.exports = class Autocore extends ReadyResource {
  ...
  _isSystem () {
    return this.base.system.core._backingCore() === this.core
  }
  ...
  _checkpoint () {
    if (this.indexing) return this._checkpointPostBatch()

    const signature = this.core.core.header.tree.signature
    const length = this.core.core.tree.length

    return { signature, length }
  }

  _checkpointPostBatch () {
    const batch = this.core.core.tree.batch()
    const blocks = this.indexBatch(0, this.indexing)
    for (const val of blocks) batch.append(val)

    return {
      signature: this.core.core.defaultAuth.sign(batch.signable(batch.hash())),
      length: this.indexing + this.core.length
    }
  }

  update (opts) {
    if (this === this.base.system.core._source) return
