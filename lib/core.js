...
module.exports = class Autocore extends ReadyResource {
  ...
  async flushWriteBatch (batch, treeLength, atom) {
    const reorg = treeLength < this.core.length

    if (reorg && batch.length === 0) this.base._viewStore._unindex(this)

    await this.core.state.overwrite(batch.state, { treeLength, atom })
  }
  ...
  async _updateCheckpoint (length) {
    ...
    batch.fork = this.core.core.tree.fork
  ...
  async flush (length, atom) {
    ...
    if (!this._shouldFlush(length)) {
      this.atom.flush()
      this.atom = null
      return true
    }

    const { valid, promise } = await this._flush(length, this.atom)
    this.atom.exit()
  ...
  async _flush (length, atom) {
    if (this.core.signedLength >= length) {
      return { valid: true, promise: null }
    }
    ...
    if (signableLength <= this.core.signedLength) {
      return { valid: false, promise: null }
    }

    const signature = await this.signer.sign(indexers, signableLength, length)

    const promise = this.originalCore.core.commit(this.core.state, {
      length: signableLength,
      signature,
      atom
    })

    return { valid: true, promise }
  }
  ...
  async migrateTo (indexers, length, atom) {
    ...
    const writes = [
      this.core.state.moveTo(core.core, length, { atom }),
      this._ensureUserData(core, true, atom),
      this._ensureUserData(this.core, true, atom)
    ]
  ...
  async catchup ({ key, length, systemIndex }, atom) {
    if (atom) atom.enter()
    ...
    const treeLength = this.core.signedLength

    const actions = [
      this.core.state.overwrite(this.originalCore.state, { length, treeLength, infill: false, atom }),
      this._ensureUserData(this.originalCore, false, atom)
    ]

    if (swap) actions.push(this.core.state.moveTo(swap.core, length, atom))
