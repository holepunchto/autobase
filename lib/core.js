...
class AutocoreSession extends EventEmitter {
  constructor (source, core, snapshot, indexed, valueEncoding) {
    ...
    this.writable = core !== null // TODO: make this configurable
  ...
  async append (block) {
    ...
    return this._core.append(buffers)
  }
...
module.exports = class Autocore extends ReadyResource {
  ...
  _createSession (snapshot, valueEncoding, { indexed, writable, checkout } = {}) {
    if (writable) {
      const core = this.core.session({ draft: true, checkout })
      return new AutocoreSession(this, core, snapshot, indexed, valueEncoding ? c.from(valueEncoding) : null)
    }

    return new AutocoreSession(this, null, snapshot, indexed, valueEncoding ? c.from(valueEncoding) : null)
  }

  async flushWriteBatch (batch) {
    const length = batch.length - batch._sourceLength
    await this.core.state.overwrite(batch._core.state)

    if (!length) return

    this.base._onviewappend(this, length)
    this._emitAppend()
  }
