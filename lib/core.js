...
class AutocoreSession extends EventEmitter {
  ...
  async get (index, opts) {
    ...
    // console.trace(index, this.length, this._source.name, this.base.local.key)
...
module.exports = class Autocore extends ReadyResource {
  ...
  async _ensureCore (key, length) {
    ...
    const core = this.originalCore.session({ name: `${this.batchCounter++}`, checkout: length })
    await core.ready()
  ...
  async reset (length) {
    let core = this.originalCore.session({ name: `${this.batchCounter++}`, checkout: length })
    await core.ready()
    if (core.length > length) {
      await core.close()
      core = this.originalCore.session({ name: `${this.batchCounter++}`, checkout: length })
      await core.ready()
    }
  ...
  async _open () {
    ...
    if (!this.core) {
      this.core = this.originalCore.session({ name: `${this.batchCounter++}`, checkout: 0 })
  ...
  async _flush () {
    ...
    if (maxFlushedLength > this.core.flushedLength && this.core.flushedLength < this.indexedLength) {
      flushed = false
    }
    ...
    if (maxSignedLength > this.core.flushedLength) {
      const signature = await this.signer.sign(indexers, maxSignedLength)
      if (!(await this.originalCore.core.commit(this.core.state, { length: maxSignedLength, signature }))) return false
      flushed = true
    }
  ...
  async _migrateCurrentCore (next, length, indexers) {
    ...
    const batch = next.session({ name: `${this.batchCounter++}`, checkout: length })
    await batch.ready()
  ...
  _truncateAndDetach (sharedLength) {
    ...
    throw new Error('TODO: snapshots')
