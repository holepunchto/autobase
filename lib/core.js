...
module.exports = class Autocore extends ReadyResource {
  ...
  createSession (valueEncoding, indexed) {
    if (this.opened === false) {
      return new Hypercore({ valueEncoding, writable: false, preload: this._preload({ indexed }) })
    }

    if (indexed) return this.originalCore.session({ valueEncoding, writable: false })
