...
module.exports = class Autocore extends ReadyResource {
  ...
  _shouldFlush () {
    ...
    return this.signer.getSignableLength() > this.core.indexedLength
  }
  ...
  async _flush (length = this.flushableLength, log) {
    if (length <= this.core.indexedLength) return !!(length && length === this.core.indexedLength)
  ...
  async _tryMigrateCore () {
    ...
    const batch = await this._migrateCurrentCore(core, indexers, length)
  ...
  async _migrateCurrentCore (next, indexers) {
    ...
    if (batch.length < this.core.length) {
      await batch.append(this.core._appends)
    }
