...
class Snapshot {
  ...
  clone () {
    ...
    snap.tipLength = this.tipLength
    snap.tip = this.tip
    return snap
  }

  getIndexedLength () {
    const sharedLength = this.length - (this.tip === null ? 0 : this.tip.length)
    return Math.min(this.source.indexedLength, sharedLength)
  }

  getIndexedByteLength () {
    const sharedByteLength = this.tip === null || this.tip.length === 0
      ? this.byteLength
  ...
  clear () {
    if (this.tip === null) this.detach(null, 0)
    this.source = null
  }
...
module.exports = class Autocore extends ReadyResource {
  ...
  async _open () {
    ...
    const expectedLength = await this.base.loadIndex(this.name)
