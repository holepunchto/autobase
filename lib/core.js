...
class AutocoreSession extends EventEmitter {
  ...
  snapshot ({ valueEncoding = this.valueEncoding } = {}) {
    if (this.closing) throw new Error('SESSION_CLOSED')
  ...
  session ({ valueEncoding = this.valueEncoding } = {}) {
    if (this.closing) throw new Error('SESSION_CLOSED')
  ...
  async get (index, opts) {
    if (this._source.opened === false) await this._source.ready()
    ...
    if (index >= this.length) {
      throw new Error('Out of bounds, will wait in the future')
    }
