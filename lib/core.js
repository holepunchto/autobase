...
module.exports = class Autocore extends ReadyResource {
  ...
  async _flush (length = this.flushableLength) {
    ...
    // only ever sign if we are on latest state
    if (!sameIndexers(this.indexers, this.base.linearizer.indexers)) return false
  ...
  async _migrateCurrentCore (next, indexers, length) {
    ...
    if (length > this.core.indexedLength) length = this.core.indexedLength
