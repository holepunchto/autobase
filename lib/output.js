...
module.exports = class Output {
  constructor (id, core, opts = {}) {
    ...
    if (!this.compressor) {
      this.compressor = new KeyCompressor(this.core)
      // Does not need to be deregistered because the core is always closed
      this.core.on('truncate', length => this.compressor.truncate(length))
    }
  }
