...
module.exports = class Consensus {
  ...
  addHead (node) {
    if (!node.writer._isActiveIndexer) return
  ...
  _isMerge (node) {
    if (!node.writer._isActiveIndexer) return false
  ...
  _acks (target) {
    const acks = target.writer._isActiveIndexer ? [target] : [] // TODO: can be cached on the target node in future (ie if we add one we dont have to check it again)
