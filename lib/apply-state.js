...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _assembleMultisig (signableLength, checkpoints) {
    ...
    for (let i = 0; i < views.length; i++) {
      const v = i === 0 ? { mappedIndex: -1, core: this.system.core, ref: this.systemRef } : this.views[i - 1]

      const checkpointIndex = i === 0 ? 0 : v.mappedIndex + 1
      const isIndexed = i === 0 || (v.mappedIndex !== -1 && v.mappedIndex < info.views.length)

      if (!v.core || !isIndexed) continue

      const length = i === 0 ? signableLength : info.views[v.mappedIndex].length

      if (length <= v.core.signedLength) continue
  ...
  async truncate (systemLength) {
    ...
    for (let i = 0; i < this.views.length; i++) {
      const { mappedIndex, core } = this.views[i]

      const length = (mappedIndex !== -1 && mappedIndex < views.length) ? views[mappedIndex].length : 0
      if (!core || core.length === length) continue
      if (length === 0) this.views[i].mappedIndex = -1
  ...
  async _signAllViewCores () {
    ...
    for (let i = 0; i < this.views.length; i++) {
      const { mappedIndex, ref } = this.views[i]
      if (mappedIndex === -1 || mappedIndex >= info.views.length) {
        promises[i + 1] = this._signViewCore(ref.atomicBatch, 0)
        continue
      }

      promises[i + 1] = this._signViewCore(ref.atomicBatch, info.views[mappedIndex].length)
    }
  ...
  async _rollbackViews () {
    for (const view of this.views) {
      if (view.mappedIndex !== -1 && view.mappedIndex < this.system.views.length) {
        view.key = this.system.views[view.mappedIndex].key
      } else {
