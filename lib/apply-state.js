  ...
  async update (u, localNodes) {
    ...
    for (let k = u.shared; k < u.length; k++) {
      ...
      for (const trace of node.trace) {
        const view = trace.view === 0 ? this.systemView : this.views[trace.view - 1]
        // Request blocks but don't await & ignore errors
        warmups.push(view.core.download({ blocks: trace.blocks }))
      }
    }
