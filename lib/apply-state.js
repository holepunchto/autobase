...
module.exports = class ApplyState extends ReadyResource {
  ...
  static async verifyBoot(base, boot) {
    ...
    try {
      ...
      const encCore = await EncryptionView.setSystemEncryption(base, batch)
      ...
      if (isViewCorruption(err)) return false
      throw err
    } finally {
  ...
  static async clearViewBatches(base, key) {
    const sysCore = base.store.get(key)
    const encCore = await EncryptionView.setSystemEncryption(base, sysCore)

    const system = new SystemView(sysCore)
    await system.ready()

    for (const view of system.views) {
      const core = base.store.get(view.key)
      await core.ready()
      await core.state.storage.deleteSessions()
      await core.close()
    }

    if (encCore) await encCore.state.storage.deleteSessions()
    await sysCore.state.storage.deleteSessions()

    if (encCore) encCore.close()
    await sysCore.close()
  }
