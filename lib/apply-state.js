...
module.exports = class ApplyState extends ReadyResource {
  ...
  async shouldWrite () {
    ...
    for (let i = 0; i < info.views.length; i++) {
      if (user[i].length < info.views[i].length) return true
    }
    ...
    // check if our system signature is sufficiently far behind
    if (this.localCheckpoint.signedLength() + this.stretchedThreshold < this.indexedLength) {
      return true
    }
