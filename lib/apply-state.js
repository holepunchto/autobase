...
const { encodeValue } = require('./encode-value.js')
...
module.exports = class ApplyState extends ReadyResource {
  ...
  async createAnchor (key, length, encryption) {
    ...
    const state = { start: 0, end: 36, buffer: b4a.alloc(36) }
    c.fixed32.encode(state, key)
    c.uint32.encode(state, length)

    const entropy = crypto.hash(state.buffer)
    const manifestData = c.encode(messages.ManifestData, { version: 0, legacyBlocks: 0, entropy })

    const padding = this.encryption ? AutobaseEncryption.PADDING : 0
    const block = encodeValue(null, { heads: [{ key, length }], padding })
    if (this.encryption) await this.encryption.encryptAnchor(block, entropy)
    ...
    if (core.length !== 1) await core.append(block, { writable: true })
