...
class CheckpointCore {
  ...
  updateInternalSignatures (length, signatures) {
    this.signatures.internal = updateSignatures(length, signatures, this.signatures.internal)
  }
  ...
  makeCheckpoints (length) {
    return {
      internal: makeCheckpoint(length, this.signatures.internal),
      user: makeCheckpoint(length, this.signatures.user)
    }
  }
  ...
  signedLength () {
    return this.signatures.internal.length ? this.signatures.internal[0].length : 0
  }
  ...
  _sameSignatures (b, a) {
    return sameSignatures(a.internal, b.internal) && sameSignatures(a.user, b.user)
  }

  async _inflateAllCheckpoints (length, checkpoints) {
    if (!checkpoints) return { internal: [], user: [] }
    if (this._sameSignatures(checkpoints, this.signatures)) return this.signatures

    const internal = new Array(checkpoints.internal.length)
    const user = new Array(checkpoints.user.length)

    for (let i = 0; i < internal.length; i++) {
      const chk = checkpoints.internal[i]
      internal[i] = this._inflateInternalCheckpoint(chk, i, length)
    }
    ...
    return {
      internal: await Promise.all(internal),
      user: await Promise.all(user)
    }
  }

  async _inflateInternalCheckpoint (chk, i, coreLength) {
    if (chk.checkpoint) {
    ...
    if (i < this.signatures.internal.length && this.signatures.internal[i].at === len) {
      return this.signatures.internal[i]
    }

    if (len <= 0) return null

    const { checkpoint } = await this.core.get(len - 1)
    if (!checkpoint || i >= checkpoint.internal.length || !checkpoint.internal[i].checkpoint) return null

    const { signature, length } = checkpoint.internal[i].checkpoint
...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _openInternalView (name) {
    ...
    const mappedIndex = this.internalViews.length

    this.internalViews.push({ ref, core, mappedIndex, indexedLength: 0 })
  ...
  async _open () {
    ...
    const sysCore = await this._openInternalView('_system')

    if (this.encryption !== null && sysCore.manifest.version >= 2) {
      // open first so encryption is ready for system
      const encryptionCore = await this._openInternalView('_encryption')
      await this.encryption.reload(encryptionCore)

      if (!this.encryption.bootstrapped && encryptionCore.manifest.version >= 2) {
        await this.encryption.update(this.base.encryptionKey)
      }
    }
    ...
    for (let i = 0; i < this.internalViews.length; i++) {
      const view = this.internalViews[i]
      view.indexedLength = i < boot.internalViews.length ? boot.internalViews[i] : 0
    }
  ...
  async maybeSigned () {
    ...
    const internalSignatures = this.checkpoints[thres].signatures.internal
    ...
    for (let i = thres; i < this.checkpoints.length; i++) {
      ...
      if (chk.signatures.length === 0 || chk.signatures.internal[0].length > this.indexedLength) {
        this.needsIndexedLengthUpdate = true
        return
      }
    }
    ...
    await this._assembleMultisig(internalSignatures, chk)
  }

  async _assembleMultisig (internalSignatures, checkpoints) {
    const views = new Array(this.views.length + internalSignatures.length)

    const signableLength = internalSignatures[0].length
    ...
    let i = 0
    for (; i < internalSignatures.length; i++) {
      const { length } = internalSignatures[i]
      const internal = this.internalViews[i]
      ...
      for (let j = 0; j < checkpoints.length; j++) {
        ...
        views[i].signatures[j] = {
          signature: signatures.internal[i].signature,
          length: signatures.internal[i].length,
          signer
        }
      }
    }
    ...
    for (; i < views.length; i++) {
      ...
      for (let j = 0; j < checkpoints.length; j++) {
        const { signer, signatures } = checkpoints[j]
        const { length, signature } = signatures.user[view.mappedIndex]
  ...
  async _close () {
    ...
    for (const v of this.internalViews) {
  ...
  _deleteUpdate (u) {
    const encoding = u.version === 0 ? messages.LinearizerKeyV0 : messages.LinearizerKey
    this.tx.deleteLocal(c.encode(encoding, u.seq))
  }

  _pushUpdate (u) {
    u.seq = this.updates.length === 0 ? 0 : this.updates[this.updates.length - 1].seq + 1
    for (const view of this.internalViews) u.internalViews.push(view.core.length)
    this.updates.push(u)
  ...
  _decodeUpdate (entry) {
    const { prefix, seq } = c.decode(messages.LinearizerKey, entry.key)

    switch (prefix) {
      case messages.LINEARIZER_PREFIX_V0: {
        const upd = c.decode(messages.LinearizerUpdateV0, entry.value)
        upd.seq = seq
        return upd
      }

      case messages.LINEARIZER_PREFIX: {
        const upd = c.decode(messages.LinearizerUpdate, entry.value)
        upd.seq = seq
        return upd
      }
    }
  ...
  async catchup (linearizer) {
    ...
    for await (const data of this.local.state.storage.createLocalStream({ gte, lt })) {
      updates.push(this._decodeUpdate(data))
    }
    ...
    if (this.indexersUpdated) {
      ...
      while (this.updates.length > 0 && this.updates[this.updates.length - 1].internalViews[0] >= this.indexedLength) {
  ...
  async undo (popped) {
    ...
    const systemLength = u ? u.internalViews[0] : this.indexedLength
  ...
  async _signInternalViewCores () {
    return Promise.all(this.internalViews.map(v => this._signViewCore(v.core, v.indexedLength)))
  }

  async _signUserViewCores () {
  ...
  async finalize (key) {
    ...
    const pointer = c.encode(messages.BootRecord, {
      key,
      internalViews: this.internalViews.map(v => v.indexedLength),
  ...
  _indexUpdates (indexed) {
    ...
    for (let i = 0; i < this.internalViews.length; i++) {
      this.internalViews[i].indexedLength = last.internalViews[i]
    }
  ...
  async update (u, localNodes) {
    ...
    for (i = u.shared; i < u.length; i++) {
      ...
      if (!optimist) {
        if (node.writer.isRemoved && !node.writer.isActiveIndexer) {
          ...
          const u = { seq: 0, key: node.writer.core.key, length: node.length, batch, internalViews: [], indexers: false }
          this._pushUpdate(u)
          batch = 0
    ...
    // todo: indexedLength may not have been updated by time we validate
    // may not even need this condition
    if (this.pendingFork && indexersUpdated > 0 && this.pendingFork.length > indexersUpdated) {
      this.pendingFork = null
    }

    if (this.pendingFork) {
      const fork = this.pendingFork
      if (fork.length > this.indexedLength) this._indexUpdates(fork.length)
    ...
    return indexersUpdated === 0 ? null : { fork: null }
  }
  ...
  async _appendLocalNodes (localNodes) {
    ...
    if (this.localIndexer) {
      ...
      if (this.indexedLength > signedLength) {
        internalSignatures = await this._signInternalViewCores()
        signatures = await this._signUserViewCores()
  ...
  async _resetView (view) {
    ...
    view.key = await this.store.createView(manifests, view.name, null, this.system.core.manifest.version, this.system.entropy, manifestData) // will never be system so no linked
    view.length = 0
  }
  ...
  async _generateNextViews () {
    ...
    const manifests = await this.store.getIndexerManifests(this.system.indexers)
    const entropy = this.system.version < 2 ? null : this.system.getEntropy(this.system.indexers)

    for (const v of this.views) {
      const manifestData = v.core ? v.core.manifest.userData : null

      const prologue = await getPrologue(v)
      const key = await this.store.createView(manifests, v.name, prologue, this.system.core.manifest.version, entropy, manifestData)
...
function makeCheckpoint (length, signatures) {
  const checkpoints = new Array(signatures.length)

  for (let i = 0; i < signatures.length; i++) {
    const chk = signatures[i]
