...
const { Migration, Fork } = require('./migrations.js')
...
module.exports = class ApplyState extends ReadyResource {
  ...
  async update (u, localNodes) {
    ...
    for (i = u.shared; i < u.length; i++) {
      ...
      if (!optimist) {
        ...
        if (applyBatch.length && this.base._hasApply === true) {
          ...
          this.applying = this.applyBatch = null
        }
      }
    ...
    if (migrated) {
      const indexedSystem = await this.system.getIndexedInfo(this.indexedLength)
      const migration = new Migration(this.base, this.store, indexedSystem, this.indexedLength)

      const key = await migration.upgrade()

      if (key === null) throw new Error('Migration failed')
