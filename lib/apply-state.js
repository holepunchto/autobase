...
class AutobaseHostCalls {
  ...
  async fork (indexerKeys, system) {
    ...
    this.state.pendingFork = { indexers: indexerKeys, length: system.length }
    return true
  }
}
...
module.exports = class ApplyState extends ReadyResource {
  ...
  shouldWrite () {
    if (!this.localCheckpoint || !this.localCheckpoint.opened) return false
  ...
  async maybeSigned () {
    ...
    for (let i = thres; i < this.checkpoints.length; i++) {
      ...
      if (!b4a.equals(expected, chk.digest.key)) {
        // TODO: use this as a signal for static ff
        return
      }
