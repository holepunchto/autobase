...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _assembleMultisig (signableLength, checkpoints) {
    ...
    for (let i = 0; i < views.length; i++) {
      const mappedIndex = i === 0 ? -1 : this.views[i - 1].mappedIndex
      const length = i === 0 ? signableLength : (mappedIndex !== -1 && mappedIndex < info.views.length) ? info.views[mappedIndex].length : 0
    ...
    await Promise.all(promises, views)
    if (this.interrupted) return
  ...
  async _rollbackViews () {
    for (const view of this.views) {
      if (view.mappedIndex !== -1 && view.mappedIndex < this.system.views.length) {
        ...
        // todo: try resetting mappedIndex here
