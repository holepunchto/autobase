...
const { OPLOG_VERSION, BOOT_RECORD_VERSION, ENCRYPTION_RECORD_VERSION } = require('./caps.js')
...
module.exports = class ApplyState extends ReadyResource {
  constructor(base) {
    ...
    this.base = base
    this.encryption = new EncryptionView(base, null)
    this.valueEncoding = base.valueEncoding
    this.store = base._viewStore.atomize(this.encryption)
    this.key = null
    this.system = null
    this.view = null
    this.views = []
  ...
  async _boot() {
    ...
    const encryption = await this.base._getEncryptionRecord()

    this.manifestVersion = boot.manifestVersion
    ...
    await this.encryption.load(this.encryptionView.core, encryption.bootstrap)
  ...
  async recoverAt() {
    ...
    for await (const { length, info } of SystemView.flushes(this.systemRef.core, {
      reverse: true,
      lt
    })) {
      for (let i = 0; i < info.views.length; i++) {
  ...
  bootstrapEncryption(bootstrap) {
    this.base.encryption.bootstrap(bootstrap)
    this.encryption.bootstrap(bootstrap)

    this.localState.updateEncryptionRecord({
  ...
  updateEncryption(payload) {
    // bump system version once encryption view is used
    if (!this.encryptionView.core.length && this.system.version < 2) {
      this.system.setVersion(2)
    }

    if (this.manifestVersion < 2) this.manifestVersion = 2

    return this.encryption.update(payload)
  }
  ...
  async update(u, localNodes) {
    ...
    const currentManifestVersion = this.manifestVersion
    ...
    for (i = u.shared; i < u.length; i++) {
      ...
      await this.system.flush(this.views, this.encryptionView.core.length)
      ...
      if (!indexed || bootStateUpdatedAt) continue

      this.base._shiftWriter(node.writer)

      if (update.indexers || this.manifestVersion !== currentManifestVersion) {
        bootStateUpdatedAt = i + 1
      }
    }
    ...
    if (this.pendingFork) {
      ...
      this.manifestVersion = fork.manifestVersion
    ...
    if (u.indexed.length) {
      this._indexUpdates(bootStateUpdatedAt || u.indexed.length)
    }

    const migrated = bootStateUpdatedAt !== 0
  ...
  async createAnchor() {
    ...
    const padding = this.base.isEncrypted ? HypercoreEncryption.PADDING : 0
    const block = encodeValue(null, { heads: [{ key, length }], padding })

    if (this.base.isEncrypted) await this.encryption.encryptAnchor(block, namespace)
