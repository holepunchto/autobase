...
const { AutobaseEncryption, EncryptionView } = require('./encryption.js')
...
module.exports = class ApplyState extends ReadyResource {
  ...
  static async verifyBoot(base, boot) {
    if (!boot) return true
    ...
    console.log('CHECKING VIEW BATCHES', boot)
    try {
      const core = base.store.get({ key: boot.key, active: false, encryption: null })
      cores.push(core)
      await core.ready()

      console.log('before check', core.manifest, core.length, core.opened)

      const batch = core.session({ name: 'batch' })
      batches.push(batch)
      await batch.ready()

      console.log('before check batch', batch.key, batch.length)

      await Hypercore.treeHashFromStorage(batch)

      const encCore = await EncryptionView.setSystemEncryption(base, core)
      if (encCore) cores.push(encCore)

      system = new SystemView(batch)
      await system.ready()

      for (const view of system.views) {
        const viewCore = base.store.get(view.key)
        const viewBatch = viewCore.session({ name: 'batch' })
      ...
      safetyCatch(err)
      console.log('CHECKED VIEW BATCHES', err, isViewCorruption(err))
      return isViewCorruption(err) === false
    } finally {
      if (system) await system.close()
      for (const core of cores) await core.close()
      for (const batch of batches) await batch.close()
    }
  }

  static async clearViewBatches(store, key) {
    console.log('CLEARING VIEW BATCHES')
    const sysCore = store.get(key)
    ...
    for (const view of system.views) {
      ...
      await core.ready()
      console.log('pre clearing viewCore', core.key, core.length)
      await core.state.storage.deleteBatches()
      await core.close()
    }

    console.log('pre clearing sysCore', sysCore.key, sysCore.length)
    await sysCore.state.storage.deleteBatches()
    await sysCore.close()
