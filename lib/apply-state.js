...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _resetView (view) {
    ...
    view.key = await this.store.createView(manifests, view.name, null, this.system.core.manifest.version, null) // will never be system so no linked
    view.length = 0
  }
  ...
  async _generateNextViews () {
    ...
    for (const v of this.views) {
      ...
      const key = await this.store.createView(manifests, v.name, prologue, this.system.core.manifest.version, null)
