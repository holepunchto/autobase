...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _open () {
    ...
    if (this.base.viewEncryption) {
      await this.base.viewEncryption.reload(sysCore)
    }
  ...
  async truncate (systemLength) {
    ...
    const updated = await this.system.update()
  ...
  async _resetView (view) {
    const manifests = await this.store.getIndexerManifests(this.system.indexers)
    view.key = await this.store.createView(manifests, view.name, null, this.system.entropy)
    view.length = 0
  }
