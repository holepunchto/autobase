...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _openInternalView (name, mappedIndex) {
  ...
  async _open () {
    ...
    const sysCore = await this._openInternalView('_system', 0)

    if (this.encryption !== null && sysCore.manifest.version >= 2) {
      // open first so encryption is ready for system
      const encryptionCore = await this._openInternalView('_encryption', 1)
    ...
    // view may be created out of order
    this.internalViews.sort(ascendingMappedIndex)
  ...
  async _resetView (view) {
    ...
    view.key = await this.store.createView(manifests, view.name, null, this.system.core.manifest.version, this.system.entropy, null) // will never be system so no linked
    view.length = 0
  }
  ...
  async _generateNextViews () {
    ...
    for (const v of this.views) {
      ...
      const key = await this.store.createView(manifests, v.name, prologue, this.system.core.manifest.version, entropy, null)
...
function ascendingMappedIndex (a, b) {
  return a.mappedIndex - b.mappedIndex
}
