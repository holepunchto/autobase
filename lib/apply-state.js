...
module.exports = class ApplyState extends ReadyResource {
  ...
  async maybeSigned () {
    ...
    for (let i = thres; i < this.checkpoints.length; i++) {
      ...
      if (chk.signatures.length === 0 || chk.signatures[0].length > this.indexedLength) return
    }
  ...
  async _appendLocalNodes (localNodes) {
    ...
    if (this.localCheckpoint) {
      ...
      if (this.indexedLength > signedLength) {
        ...
        signedAt = signedLength === 0
          ? local.length + 1 // if never signed, lets put the sigs at block 0
          : local.length + blocks.length // otherwise at the end of the batch
      }
