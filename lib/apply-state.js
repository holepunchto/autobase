...
module.exports = class ApplyState extends ReadyResource {
  ...
  _endTrace (node) {
    ...
    const blocks = this.systemRef.tracer.end()
    if (blocks.length) trace.push({ view: 0, blocks })

    for (let i = 0; i < this.views.length; i++) {
      const blocks = this.views[i].ref.tracer.end()
      if (blocks.length) trace.push({ view: i + 1, blocks })
    }

    if (trace.length) node.trace = trace
  }
  ...
  async update (u, localNodes) {
    ...
    // Organize by view
    const warmupPerView = new Array(this.views.length + 1)
    for (let k = u.shared; k < u.length; k++) {
      ...
      for (const trace of node.trace) {
        ...
        if (trace.view > this.views.length) {
          console.warn('Oplog block trace contains OOB view index', 'trace view', trace.view, 'views.length', this.views.length)
          continue
        }

        warmupPerView[trace.view] = warmupPerView[trace.view] || []
        warmupPerView[trace.view].push(...trace.blocks)
      }
    }

    // Request per view
    for (let k = 0; k < warmupPerView.length; k++) {
      const blocks = warmupPerView[k]
      if (!blocks) continue

      const view = k === 0 ? this.systemView : this.views[k - 1]
    ...
    for (i = u.shared; i < u.length; i++) {
      ...
      const attemptOptimistic = node.writer.isRemoved && node.optimistic && batch === 1 &&
        this.base._hasOptimisticApply === true

      if (node.writer.core.writable || attemptOptimistic) this._startTrace(node)

      const optimist = attemptOptimistic && await this._optimisticApply(u, node, indexed)
      ...
      if (node.writer.core.writable || attemptOptimistic) this._endTrace(node)
