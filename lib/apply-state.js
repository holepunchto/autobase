    ...
    try {
      ...
      if (isViewCorruption(err)) {
        for (const v of this.store.opened) {
          await v.core.ready()
          await v.core.state.storage.deleteSessions()
        }
        if (this.systemView) {
          const core = this.systemView.ref.core
          await core.ready()
          await core.state.storage.deleteSessions()
        }
        if (this.encryptionView) {
          const core = this.encryptionView.ref.core
          await core.ready()
          await core.state.storage.deleteSessions()
        }

        await LocalState.clear(this.base.local)
      }
...
function isAssertion(err) {
  return err.code === 'ERR_ASSERTION' || err.name === 'AssertionError'
}
