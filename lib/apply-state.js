...
module.exports = class ApplyState extends ReadyResource {
  ...
  get systemRef () {
    return this.internalViews[0].ref
  }
  ...
  pendingIndexedLength () {
    let indexed = this._applying.indexed.length
    if (!this._applying || !this.updates.length || !indexed) return this.indexedLength
  ...
  async update (u, localNodes) {
    ...
    if (indexersUpdated === 0) return { reboot: false }
  ...
  async _generateNextViews () {
    ...
    for (const v of this.views) {
      ...
      const key = await this.store.createView(manifests, v.name, prologue, this.system.core.manifest.version, entropy, null, manifestData)
