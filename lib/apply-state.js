...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _open () {
    ...
    // open first so encryption is at index 0 in system views
    const encryptionCore = this.store.get({ name: '_encryption' })
    await encryptionCore.ready()
    await encryptionCore.setUserData('referrer', this.base.key)
    await encryptionCore.setUserData('autobase/view', b4a.from('_encryption'))

    if (this.encryption !== null) {
      // open encryption view
      await this.encryption.reload(encryptionCore)

      if (!this.encryption.bootstrapped && system.version >= 2) {
  ...
  async _assembleMultisig (signableLength, checkpoints) {
    ...
    for (let i = 0; i < views.length; i++) {
      ...
      for (let j = 0; j < checkpoints.length; j++) {
        ...
        const { length, signature } = signatures[viewIndex]
  ...
  async _signAllViewCores () {
    ...
    const promises = new Array(sys.views.length + 1)
    ...
    for (let i = 0; i < this.views.length; i++) {
      ...
      const viewIndex = view.mappedIndex + 1
