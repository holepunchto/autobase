...
module.exports = class ApplyState extends ReadyResource {
  ...
  _decodeUpdate (entry) {
    const seq = c.decode(messages.LinearizerKey, entry.key)
  ...
  async finalize (key) {
    const internalViews = new Array(this.internalViews.length)

    for (let i = 0; i < internalViews.length; i++) {
      internalViews[i] = this.internalViews[i].indexedLength
    }
