...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _open () {
    ...
    for (let i = 0; i < system.views.length; i++) {
      ...
      if (v === null) {
        this.views.push({ name: null, key, length, core: null, ref: null, mappedIndex: i, force: false })
        continue
      }
      ...
      const force = this._forceView(v.name)

      this.views.push({ name: v.name, key, length, core: v.atomicBatch, ref: v, mappedIndex: i, force })
    ...
    for (const v of this.store.opened) {
      ...
      this.views.push({ name: v.name, key: core.key, length: core.length, core, ref: v, mappedIndex: -1, force })
    }
  ...
  _forceView (name) {
    return this.system.version >= 2 && name === '_encryption'
  }
