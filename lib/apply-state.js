...
module.exports = class ApplyState extends ReadyResource {
  ...
  async catchup (linearizer) {
    ...
    const updatesBuffer = await this.store.getLocal().getUserData('autobase/linearizer')
    const updates = updatesBuffer ? c.decode(messages.LinearizerUpdateArray, updatesBuffer) : []
  ...
  async _flush (localNodes) {
    ...
    await local.setUserData('autobase/boot', pointer)
    await local.setUserData('autobase/linearizer', c.encode(messages.LinearizerUpdateArray, this.updates))
    ...
    for (i = u.shared; i < u.length; i++) {
      ...
      if (!optimist) {
        if (node.writer.isRemoved && !node.writer.isActiveIndexer) {
          ...
          this.updates.push({ key: node.writer.core.key, length: node.length, batch, systemLength: this.system.core.length, indexers: false })
          batch = 0
