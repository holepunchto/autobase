...
class UpdateChanges {
  constructor (applyState) {
    this.applyState = applyState
    this.byName = new Map()
  }

  get discoveryKey () {
    return this.applyState.discoveryKey
  }
  ...
  get id () {
    return this.applyState.id
  }

  get system () {
    return this.applyState.system
  }

  truncated () {
    for (const v of this.applyState.views) {
      const shared = v.atomicBatch ? v.atomicBatch.length : 0
      this.byName.get(v.ref.name).shared = shared
    }
    this.byName.get('_system').shared = this.applyState.systemRef.atomicBatch.length
  }

  start () {
    for (const v of this.applyState.views) {
      if (!v.ref) continue
      const from = v.atomicBatch ? v.atomicBatch.length : 0
      this.byName.set(v.ref.name, {
        shared: from,
        from,
        to: from
      })
    }
    const from = this.applyState.systemRef.atomicBatch.length
    this.byName.set('_system', {
      shared: from,
      from,
      to: from
    })
    ...
    for (const v of this.applyState.views) {
      if (!v.ref) continue
      const to = v.atomicBatch ? v.atomicBatch.length : 0
      this.byName.get(v.ref.name).to = to
    }
    this.byName.get('_system').to = this.applyState.systemRef.atomicBatch.length
  }
...
module.exports = class ApplyState extends ReadyResource {
  constructor (base) {
    ...
    this.systemRef = null
    this.hostcalls = null
    this.changes = base._hasUpdate ? new UpdateChanges(this) : null
  ...
  async truncate (systemLength) {
    ...
    if (this.changes !== null) this.changes.truncated()
  }
  ...
  async update (u, localNodes) {
    if (this.changes !== null) this.changes.start()
    if (this.tx === null) this.tx = this.local.state.storage.write()
