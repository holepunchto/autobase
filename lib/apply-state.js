...
class CheckpointCore {
  ...
  async _inflateEncryptionCheckpoint (chk, coreLength) {
    if (!chk) return { length: 0, signature: null }
...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _open () {
    ...
    this.encryptionView = await this._openInternalView('_encryption', boot.encryptionLength)
  ...
  async maybeSigned () {
    ...
    const { system, encryption } = this.checkpoints[thres].signatures

    await this._assembleMultisig(system.length, encryption.length, chk)
  }

  async _assembleMultisig (signableLength, encryptionLength, checkpoints) {
    ...
    if (encryptionLength) {
      views[1] = createCheckpointSignature(encryptionLength, this.encryptionView, checkpoints.length)
    }
  ...
  _pushUpdate (u) {
    ...
    u.encryptionLength = this.encryptionView.core.length
    this.updates.push(u)
  ...
  async _signInternalViewCores () {
    return {
      ...
      encryption: await this._signViewCore(this.encryptionView.core, this.encryptionView.indexedLength)
    }
  }
  ...
  async finalize (key) {
    this.localState.setBootRecord({
      ...
      encryptionLength: this.encryptionView.indexedLength,
  ...
  _indexUpdates (indexed) {
    ...
    this.encryptionView.indexedLength = last.encryptionLength
    ...
    for (i = u.shared; i < u.length; i++) {
      ...
      if (!optimist) {
        if (node.writer.isRemoved && !node.writer.isActiveIndexer) {
          ...
          const u = { seq: 0, key: node.writer.core.key, length: node.length, batch, systemLength: 0, encryptionLength: 0, indexers: false }
          this._pushUpdate(u)
          batch = 0
...
function updateSignatures (length, signatures, existing) {
  ...
  for (let i = 0; i < updated.length; i++) {
    updated[i] = updateSignature(length, signatures[i], existing[i])
  }
