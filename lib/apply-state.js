...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _open () {
    ...
    const encCore = this.store.get({ name: '_encryption' })
    const encryption = new EncryptionView(this.base, encCore)
    ...
    this.system = system
    this.encryptionView = encryption
    ...
    for (let i = 0; i < system.views.length; i++) {
      ...
      const v = await this.store.findViewByKey(key, system.indexers, system.entropy)
    ...
    for (const v of this.store.opened) {
      ...
      this.views.push({ name: v.name, key: v.atomicBatch.key, length: v.atomicBatch.length, core: v.atomicBatch, ref: v })
    }
  ...
  async _close () {
    ...
    if (this.encryptionView) await this.encryptionView.close()
  ...
  async _signAllViewCores () {
    ...
    const promises = new Array(info.views.length + 1)

    promises[0] = this._signViewCore(this.system.core, this.indexedLength)

    for (let i = 1; i < promises.length; i++) {
      const core = this.views[i - 1].core
      promises[i] = core ? this._signViewCore(core, info.views[i - 1].length) : null
    }
  ...
  async update (u, localNodes) {
    ...
    if (!this.encryptionView.core.length) {
      await this.encryptionView.update(this.base.encryptionKey)
    }
    ...
    if (this.pendingFork) {
      ...
      if (fork && fork.length > this.indexedLength) this._indexUpdates(fork.length)
  ...
  async _generateNextViews () {
    ...
    for (const v of this.views) {
      ...
      const key = await this.store.createView(manifests, v.name, prologue, entropy)

      v.key = key
    }
  }

  async _generateForkViews (indexers, manifests, length) {
    const { views } = await this.system.getIndexedInfo(length)
    const entropy = this.system.getEntropy(indexers)

    for (let i = 0; i < views.length; i++) {
      const view = await this.store.findViewByKey(views[i].key)
      const prologue = await getPrologue(view, views[i].length)
      views[i].key = await this.store.createView(manifests, this.views[i].name, prologue, entropy)
    }
...
async function getPrologue (view, prologueLength = -1) {
  const length = prologueLength !== -1 ? prologueLength : view.core ? view.core.length : view.length
