...
module.exports = class Signer {
  ...
  open () {
    ...
    for (const idx of this.base.linearizer.indexers) {
      for (const checkpoint of idx.flushCheckpoints(this.core.likelyIndex)) {
  ...
  async getSignableLength (indexers = this.core.indexers) {
  ...
  getSignatures (indexers, length) {
    ...
    for (let signer = 0; signer < indexers.length; signer++) {
      const checkpoint = this.bestCheckpoint(indexers[signer], true)
      if (!checkpoint) continue
