...
module.exports = class KeyCompressor {
  ...
  async decompress (compressedClock, seq, opts) {
    const clock = new Map()
    ...
    const blocks = await Promise.all(seqs.map(seq => this.core.get(seq, opts)))
  ...
  async compress (clock, seq, opts = {}) {
    ...
    if (!this.initialized && this.core.length > 0) {
      ...
      for (const clock of head.clocks) {
        await this.decompress(clock.clock, this.core.length - 1, opts)
      }
    }
  ...
  async resolvePointer (keyPointer, opts) {
    let keys = this.bySeq.get(keyPointer.seq)
    if (keys) return keys[keyPointer.offset]
    const block = await this.core.get(keyPointer.seq, opts)
