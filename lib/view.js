...
module.exports = class ViewState extends ReadyResource {
  ...
  async _signCoreAt (core, key, len) {
    const batch = await core.restoreBatch(len)
    const signature = crypto.sign(batch.signable(key), this.base.local.keyPair.secretKey)
    return signature
  }
  ...
  async _checkpoint () {
    ...
    checkpoint.push({
      checkpointer: 0,
      signature: await this._signCoreAt(this.system.core, this.systemKey, this.indexedLength)
    })
    ...
    for (let i = 0; i < info.views.length; i++) {
      ...
      if (core) {
        checkpoint.push({
          checkpointer: 0,
          signature: await this._signCoreAt(core, info.views[i].key, info.views[i].length)
        })
      } else {
        checkpoint.push({
          checkpointer: 0,
  ...
  async _appendLocalNodes (localNodes) {
    const checkpoint = this.isSigner ? await this._checkpoint() : null
