...
module.exports = class ViewState extends ReadyResource {
  ...
  async _open () {
    const boot = await this.base._getBootInfo()
    ...
    for (let i = 0; i < boot.indexers.length; i++) {
      const idx = boot.indexers[i]
  ...
  async maybeSigned () {
    ...
    for (let i = thres; i < this.checkpoints.length; i++) {
      ...
      if (!b4a.equals(expected, chk.digest)) return
  ...
  async preupdate () {
    if (this.opened === false) await this.ready()
    if (await this.system.update()) await this.refreshWriters()
  }
  ...
  async update (u, localNodes) {
    ...
    for (i = u.shared; i < u.length; i++) {
      ...
      const v = (await this.system.get(node.writer.core.key)) || { length: 0 }
      if (node.length !== v.length + batch) {
        console.log('no', node.length, v, batch, node.writer.core.key, node.writer.core.writable, batch, this.system.indexers, this.system._clockUpdates)
  ...
  async _appendLocalNodes (localNodes) {
    ...
    const digest = { pointer: 0, key: this.system.core.key }
