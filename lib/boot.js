...
module.exports = async function boot(
  ...
  if (encryptionKeyEncryptedBuffer) {
    const { value, rotated } = await encryptionEncoding.decrypt(
      encryptionKeyEncryptedBuffer,
      blindEncryption.decrypt
    )

    if (rotated) {
      const encrypted = await encryptionEncoding.encrypt(value, blindEncryption.encrypt)
      await result.local.setUserData('autobase/blind-encryption', encrypted)
    }
  ...
  if (!result.encryptionKey && (encryptionKey || encrypt)) {
    ...
    if (blindEncryption) {
      const encrypted = await encryptionEncoding.encrypt(encryptionKey, blindEncryption.encrypt)
      await result.local.setUserData('autobase/blind-encryption', encrypted)
    } else {
