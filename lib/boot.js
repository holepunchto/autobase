...
module.exports = async function boot (corestore, key, { encrypt, encryptionKey, keyPair, local } = {}) {
  ...
  if (key) {
    ...
    const local = local ? local.session({ active: false, exclusive: true, valueEncoding: messages.OplogMessage }) : getLocalCore(corestore, keyPair)
    await local.ready()

    const key = await local.getUserData('referrer')
    if (key) {
      result.key = key
      ...
      result.bootstrap = local.session({ active: false, valueEncoding: messages.OplogMessage })
      await result.bootstrap.setUserData('autobase/local', local.key)
    }
