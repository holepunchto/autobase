...
module.exports = async function boot(
  ...
  const [encryptionKeyBuffer, encryptionRecord, pointer] = await Promise.all([
    result.local.getUserData('autobase/encryption'),
    result.local.getUserData('autobase/encryption-record'),
  ...
  if (encryptionRecord) {
    result.encryption = c.decode(messages.EncryptionRecord, encryptionRecord)
  } else if (encryptionKeyBuffer) {
    result.encryption = {
  ...
  if (!result.encryption?.encryptionKey && (encryptionKey || encrypt)) {
    if (!encryptionKey)
    ...
    result.encryption = { encryptionKey, bootstrap: null }
    ...
    await result.local.setUserData(
      ...
      c.encode(messages.EncryptionRecord, result.encryption)
    )
  }
