...
module.exports = async function boot (corestore, key, { encrypt, encryptionKey, keyPair, exclusive = true } = {}) {
  ...
  if (key) {
    ...
    if (keyPair) {
      ...
      if (bootstrap.writable && !localKey) {
        ...
        const local = localKey
          ? corestore.get({ key: localKey, active: false, exclusive, valueEncoding: messages.OplogMessage })
          : corestore.get({ name: 'local', active: false, exclusive, valueEncoding: messages.OplogMessage })
    ...
    result.local = keyPair
      ? corestore.get({ keyPair, manifest, active: false, exclusive, valueEncoding: messages.OplogMessage })
      : corestore.get({ name: 'local', active: false, exclusive, valueEncoding: messages.OplogMessage })
