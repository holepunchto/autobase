...
module.exports = class RebasedHypercore {
  ...
  async _update () {
    ...
    const rebasers = []
    for (const index of [...this.indexes, this]) {
  ...
  async get (seq, opts) {
    ...
    const block = (seq < this._bestIndexLength)
      ? IndexNode.decode(await this._bestIndex.get(seq, opts))
      : this._changes[seq - this._bestIndexLength]
    if (this._unwrap) return block.value
  ...
  async commit () {
    ...
    console.log('IN COMMIT:', this._changes.length, this._bestIndexLength, this._bestIndex.length)
    ...
    console.log('after appends', this._bestIndexLength, this._bestIndex.length)
