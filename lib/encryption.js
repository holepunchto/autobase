...
class WriterEncryption extends AutobaseEncryption {
  ...
  blockKey (entropy, ctx) {
    if (ctx.manifest.userData) {
      ...
      if (userData.entropy !== null) {
        return this.encryption.blockKey(entropy, { key: userData.entropy })
      }
    }
...
class EncryptionView extends ReadyResource {
  ...
  async encryptAnchor (block, entropy) {
    const namespace = await this.get(this.id)
    if (!namespace) return null

    const blockKey = this.blockKey(namespace, { key: entropy })
