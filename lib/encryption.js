...
class WriterEncryption {
  ...
  _load (context) {
    this.keys = HypercoreEncryption.deriveKeys(this.base.encryptionKey, context.key, false, false)
  ...
  encrypt (index, block, fork, context) {
    ...
    sodium.crypto_generichash(padding, block, this.hashKey)

    // Encode padding
    c.uint64.encode({ start: 8, end: 16, buffer: padding }, 0) // reserved for key id
