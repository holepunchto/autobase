...
module.exports = class ViewEncryption extends ReadyResource {
  ...
  get core () {
    return this._core || this.base._applyState.system.core
  }
  ...
  async _getKey (name, encryptionId) {
    ...
    const key = this._generateEncryptionKey(entropy)

    this.keys.set(encryptionId, key)

    return this._keyInfo(key, name, encryptionId)
  }
  ...
  _defaultEncryption (name) {
    return this._keyInfo(this.encryptionKey, name, 0)
  }

  _keyInfo (encryptionKey, name, id) {
    return {
      version: 1,
      padding: 16,
      key: getBlockKey(this.base.bootstrap, encryptionKey, name)
    }
  }
