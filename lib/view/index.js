...
class LinearizedView extends ReadyResource {
    ...
    while (retries-- > 0) {
      try {
        ...
        if (err.code === 'SNAPSHOT_NOT_AVAILABLE') {
          await this._rebuildLocked(this.clock)
        } else {
          throw err
        }
      }
  ...
  async _headClock (id) {
    ...
    const length = this._coreLength(core)
    const head = await this._coreInnerGet(core, length - 1)
    // TODO: Remove this unnecessary wrapping
    return head ? { clock: head.clock } : { clock: new Map() }
  }
  ...
  async _update () {
    ...
    const clock = await this.autobase.latest({ wait: this.opened })
    ...
    const result = await this._rebuildLocked(clock, { wait: this.opened })
    return result
  }
  ...
  async _applyBatch (batch, node) {
    ...
    for (let id = 0; id < this.autobase._viewCount; id++) {
      if (reuseClocks[id]) {
        clocks[id] = await this._headClock(id)
      } else {
  ...
  async _apply (pending, outputsTracker) {
    if (!this.userView) {
    ...
    while (pending.length) {
      ...
      await this._applyBatch(batch, node, outputsTracker)
      batch = []
    }
  ...
  async _intersectLatest (causalStream, memoryTracker, outputsTracker, opts) {
    const pending = []
    let head = null
    ...
    for await (const node of causalStream) {
      if (!head) head = node
      latestMemory = memoryTracker.intersect(node, opts)
      if (latestMemory) break
      if (!outputsTracker.invalid) {
        latestOutputs = await outputsTracker.intersect(node, opts)
  ...
  _intersectLast (outputsTracker, opts) {
    if (!this._head) return null
    return outputsTracker.intersect(this._head, { ...opts, update: false })
  }
  ...
  async _rebuild (clock, opts = {}) {
    ...
    const causalStream = this.autobase.createCausalStream({ ...opts, clock })
    ...
    let { head, pending, latestOutputs, latestMemory } = await this._intersectLatest(causalStream, memoryTracker, outputsTracker, opts)
    let lastOutputs = null

    if (this.autobase.isIndexing && !latestOutputs) {
      // If we're indexing and we did not intersect with our own outputs, then all output should be truncated (version mismatch)
      for (const output of this.autobase._localOutputs()) {
    ...
    if (latestOutputs) {
      ...
      lastOutputs = await this._intersectLast(outputsTracker, opts)
      this.nodes = []
    ...
    try {
      await this._apply(pending, outputsTracker, opts)
    } catch (err) {
      safetyCatch(err)
      await this._rollback(this.clock, opts)
    }
  ...
  async _rebuildLocked (clock, opts) {
    const release = await this._rebuildLock()
    try {
      return await this._rebuild(clock, opts)
    } finally {
      release()
    }
  }
  ...
  _persist (deltas) {
    ...
    for (let id = 0; id < deltas.length; id++) {
      const delta = deltas[id]
      const outputs = this.autobase._localOutputs()
