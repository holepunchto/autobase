...
class LinearizedView {
  ...
  _coreAppend (session, block, opts) {
    console.log(this.__debugTag, '### IN CORE APPEND')
  ...
  async _applyBatch (batch, node) {
    ...
    try {
      console.log(this.__debugTag, this.autobase.__debugTag, '## APPLYING')
    ...
    for (let id = 0; id < clocks.length; id++) {
      if (reuseClocks[id] && this.outputs) {
        clocks[id] = this.outputs[id].clocks[id]
      }
      else {
  ...
  _persist (deltas) {
    ...
    for (let id = 0; id < deltas.length; id++) {
      ...
      console.log('persisting into:', output.core)
