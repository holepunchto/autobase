...
class LinearizedView {
    ...
    while (retries-- > 0) {
      try {
        ...
        if (err.code === 'SNAPSHOT_NOT_AVAILABLE') {
          await this._rebuildLocked(this.clock, { coreSnapshots: this.outputs.coreSnapshots })
        } else {
          throw err
        }
      }
  ...
  async _update () {
    ...
    const result = await this._rebuildLocked(clock)
    return result
  }
  ...
  async _applyBatch (batch, node, outputsTracker) {
    this._applying = node
  ...
  async _rebuild (clock, opts = {}) {
    ...
    try {
      await this._apply(pending, outputsTracker)
    } catch (err) {
