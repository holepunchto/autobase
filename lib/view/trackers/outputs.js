...
class MultiOutputSnapshot {
  ...
  _createBisector (core, clock, operations) {
    console.log('CREATING BISECTOR FOR CORE:', core.length)
    return new HypercoreBisector(core, {
      ...
      validate (node) {
        console.log('validating node:', node.operations, 'to ops:', operations, 'clock:', clock)
        if (operations > node.operations) return false
        console.log('got past operations check, node:', node)
        if (operations === node.operations && !anyEqual(node.clocks, clock, operations)) return false
        return true
      },
      cmp (node) {
        console.log('comparing node:', node.operations, 'to ops:', operations)
  ...
  async update (node) {
    console.log('bisecting for node:', node)
...
function anyEqual (clocks, clock, operations) {
  for (const { clock: nodeClock, operations: nodeOperations } of clocks) {
    if (nodeOperations !== operations) continue
