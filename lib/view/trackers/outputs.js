...
class MultiOutputSnapshot {
  ...
  _createBisector (core, clock) {
    return new HypercoreBisector(core, {
      ...
      validate (node) {
        console.log('VALIDATING NODE:', node, 'WITH VALUE:', node.value.toString(), 'AGAINST CLOCK:', clock)
        if (maybeLte(clock, node.clock)) {
          console.log('RETURNING TRUE BECAUSE CLOCK IS MAYBE LTE')
          return true
        }
      ...
      cmp (node) {
        console.log('COMPARING NODE:', node)
        if (eq(node.clock, clock)) {
          console.log('!!! FOUND INTERSECTION AT:', node.value.toString())
          return 0
        }
        if (maybeLte(clock, node.clock)) {
          return -1
  ...
  async _findCheckout (primaryIdx, idx, core, clocks) {
    ...
    console.log('BISECT VALUE AFTER SEARCH:', bisector.value)
  ...
  async intersect (node, opts = {}) {
    console.log('bisecting for node:', node, 'in cores:', this.cores.length)
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      intersectionLengths[idx] = checkout.length
      intersectionClocks[idx] = checkout.clocks
    }
...
class OutputsTracker {
  ...
  async update (node) {
    ...
    const statusByKey = await this.intersect(node)
  ...
  length (id) {
    if (!this._activeSnapshot) throw new Error('Outputs tracker does not have an active snapshot')
    console.log('active snapshot lengths:', this._activeSnapshot.lengths)
