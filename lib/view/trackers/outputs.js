...
class MultiOutputSnapshot {
  ...
  _createBisector (core, clock) {
    console.log('CREATING BISECTOR OVER CORE WITH LENGTH:', core.length)
    return new HypercoreBisector(core, {
      ...
      validate (node) {
        console.log('VALIDATING NODE:', node.clock, 'TO CLOCK:', clock)
        if (!node || maybeLte(clock, node.clock)) return true
        return false
      },
      cmp (node) {
        console.log('COMPARING NODE:', node.clock, 'TO CLOCK:', clock)
...
class OutputsTracker {
  ...
  async _validateHeaders (key) {
    ...
    for (const output of snapshots) {
      ...
      blockPromises.push(output.core.get(0))
    }
    ...
    for (const [key, { status, lengths }] of statusByKey) {
      switch (status) {
        case IntersectionStatus.Found:
          ...
          console.log('RETURNING LENGTHS:', lengths)
          return lengths
