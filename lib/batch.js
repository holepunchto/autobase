...
class MemberBatch {
  constructor (autobase) {
    ...
    this._inputsModified = false
    this._ops = []
  }

  addInput (core) {
    this._inputsModified = true
  ...
  async commit () {
    ...
    if (this._inputsModified) {
      this.autobase.inputs.sort((i1, i2) => b.compare(i1.key, i2.key))
      this.autobase._getLatestClock()
    }
