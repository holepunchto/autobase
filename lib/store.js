...
class AutoStore {
  ...
  getEncryption (name) {
    ...
    return this.base.encryption.get(name)
  }
  ...
  getViewCore (indexerManifests, name, prologue, linked) {
    const manifest = this._createManifest(indexerManifests, name, prologue, linked)
  ...
  async createView (indexerManifests, name, prologue, linked) {
    const manifest = this._createManifest(indexerManifests, name, prologue, linked)
  ...
  _freshSystemPreload (indexerManifests) {
    return {
      manifest: this._createSystemManifest(indexerManifests, null),
      exclusive: true,
  ...
  getBootstrapSystemKey () {
    return Hypercore.key(this._createSystemManifest([], null))
  }
  ...
  _createManifest (indexerManifests, name, prologue, linked) {
  ...
  _createSystemManifest (indexerManifests, prologue) {
    if (prologue !== null) throw new Error('Can only derive fresh system core')

    const encManifest = this._createManifest(indexerManifests, '_encryption', null, [])
    const linked = [Hypercore.key(encManifest)]

    return this._createManifest(indexerManifests, '_system', null, linked)
  }

  getBlockEncryption (name) {
    const key = AutobaseEncryption.getBlockKey(this.base.key, this.base.encryptionKey, name)
