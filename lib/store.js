...
class ViewCore {
  ...
  async getPrologue (prologueLength = -1) {
    const length = prologueLength !== -1 ? prologueLength : this.core ? this.core.length : this.length
    if (!length) return null

    const hash = await this.core.treeHash(length)
...
class AutoStore {
  ...
  getEncryption (name) {
    ...
    if (name === '_system') return this.getSystemEncryption()

    return new BlockEncryption({
      id: 1,
      get: id => this.base._applyState.encryptionView._getKey(name, id)
    })
  }

  getViewCore (indexerManifests, name, prologue, entropy) {
    const manifest = this._createManifest(indexerManifests, name, prologue, entropy)
  ...
  async createView (indexerManifests, name, prologue, entropy) {
    const manifest = this._createManifest(indexerManifests, name, prologue, entropy)
  ...
  async _preload (name) {
    ...
    const boot = (await this.base._getSystemInfo()) || { key: this.getBootstrapSystemKey(), indexers: [], views: [], entropy: SystemView.genesisEntropy }
    ...
    if (name === '_system') {
      return {
        key: boot.key,
        exclusive: false,
        encryption: this.getSystemEncryption()
      }
    }
  ...
  _freshCorePreload (indexerManifests, name, entropy) {
    return {
      manifest: this._createManifest(indexerManifests, name, null, entropy),
      exclusive: true,
      encryption: this.getEncryption(name)
    }
  }
  ...
  async findViewByKey (key, indexers, entropy) {
    for (const v of this.byName.values()) {
    ...
    for (const v of this.byName.values()) {
      const manifest = this._createManifest(indexerManifests, v.name, null, entropy)
  ...
  _createManifest (indexerManifests, name, prologue, entropy) {
