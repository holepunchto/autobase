...
module.exports = class AutoStore {
  ...
  get (opts, moreOpts) {
    ...
    const core = this.base.store.get({ name: 'view/' + name, cache: opts.cache, exclusive: true, encryptionKey: this.base.encryptionKey })
    const ac = new Autocore(this.base, core.batch({ autoClose: false, session: false }), name)
