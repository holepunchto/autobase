  ...
  * [Symbol.iterator] () {
  ...
  async flush (atom) {
    ...
    for (const [ac, core] of this.active) {
      ...
      flushing.push(ac.flushWriteBatch(core, treeLength, atom))
    }
...
module.exports = class AutoStore {
  ...
  async migrate (sys, atom) {
    ...
    for (const ac of this.opened.values()) {
      ...
      const length = view ? view.length : ac._isSystem() ? sys.core.length : 0
      migrating.push(ac.migrateTo(nextIndexers, length, atom))
    }

    atom.exit()
    await Promise.all(migrating)
