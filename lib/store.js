...
module.exports = class AutoStore {
    ...
    const core = indexers.length
      ...
      : this.base.store.get({
        preload: async () => {
          ...
          ac.indexers = this.base.linearizer.indexers.slice()
          if (ac.indexers.length === 1 && !ac.indexers[0].core.manifest) ac.indexers = []
  ...
  getCore (ac, indexers, opts = {}) {
    return this.base.store.get(this._corePreload(indexers, ac.name, opts.cache))
  }
  ...
  deriveNamespace (name, entropy) { // TODO: mix in the a cap of the encryption key also so the name is blinded
    return crypto.hash([NS_SIGNER_NAMESPACE, this.base.bootstrap, entropy, b4a.from(name)])
  }
  ...
  _createManifest (indexers, name) {
    ...
    const signers = indexers.map(idx => ({
      namespace: this.deriveNamespace(name, idx.core.manifest.signer.namespace),
      signature: 'ed25519',
      publicKey: idx.core.manifest.signer.publicKey
    }))
