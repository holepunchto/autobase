...
class AutoStore {
  ...
  getViewCore (indexerManifests, name, prologue, manifestVersion, entropy, linked) {
    const manifest = this._createManifest(indexerManifests, name, prologue, manifestVersion, entropy, linked)
  ...
  async createView (indexerManifests, name, prologue, manifestVersion, entropy, linked) {
    const manifest = this._createManifest(indexerManifests, name, prologue, manifestVersion, entropy, linked, null)
  ...
  async _preload (name) {
    ...
    const boot = (await this.base._getSystemInfo()) || { key: this.getBootstrapSystemKey(), indexers: [], views: [], entropy: SystemView.GENESIS_ENTROPY }
  ...
  _freshCorePreload (indexerManifests, name, entropy) {
    ...
    return {
      manifest: this._createManifest(indexerManifests, name, null, MANIFEST_VERSION, entropy, []),
      exclusive: true,
      encryption: this.getEncryption(name)
    }
  }
  ...
  _freshSystemPreload (indexerManifests, entropy) {
    return {
      manifest: this._createSystemManifest(indexerManifests, null, MANIFEST_VERSION, entropy),
      exclusive: true,
  ...
  getBootstrapSystemKey () {
    return Hypercore.key(this._createSystemManifest([], null, MANIFEST_VERSION, SystemView.GENESIS_ENTROPY))
  }
  ...
  async findViewByKey (key, indexers, manifestVersion, entropy) {
    ...
    for (const v of this.byName.values()) {
      const manifest = this._createManifest(indexerManifests, v.name, null, manifestVersion, entropy, [], null)
  ...
  _createManifest (indexerManifests, name, prologue, version, entropy, linked) {
  ...
  _createSystemManifest (indexerManifests, prologue, version, entropy) {
    if (prologue !== null) throw new Error('Can only derive fresh system core')

    const linked = []

    if (version > MANIFEST_VERSION) {
      const encManifest = this._createManifest(indexerManifests, '_encryption', null, version, entropy, [], null)
      linked.push(Hypercore.key(encManifest))
    }

    return this._createManifest(indexerManifests, '_system', null, version, entropy, linked, null)
  }
}
