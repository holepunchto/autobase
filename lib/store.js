...
class ViewCore {
  constructor(name, core, base) {
    ...
    this.ontruncate = (length) => {
      this.shared = Math.min(this.shared, length)
    }
  ...
  getNextKey() {
    return this.moveTo ? this.moveTo[0].key : null
  }
  ...
  async release() {
    ...
    while (this.atomicBatch.state.sessions.length) {
      await this.atomicBatch.state.sessions[0].close()
    }
  ...
  _getAtomicBatch(atom) {
    if (this.atomicBatch === null) {
      ...
      this.atomicBatch.on('truncate', this.ontruncate)
    }
  ...
  createSession(atom, valueEncoding) {
    if (this.batch === null) {
      ...
      this.batch.on('truncate', this.ontruncate)
    }
...
class AutoStore {
  ...
  async _preload(name) {
    ...
    console.log('run preload after preopen')
