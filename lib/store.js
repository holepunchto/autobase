...
class ViewCore {
  ...
  _getAtomicBatch(atom, encryption) {
    if (this.atomicBatch === null) {
      this.atomicBatch = this.batch.session({ atom, writable: true, encryption })
    }
  ...
  createSession(atom, valueEncoding, encryption) {
    if (this.batch === null) {
      this.batch = this.core.session({ name: 'batch', writable: true })
    }

    const s = atom
      ? this._getAtomicBatch(atom, encryption).session({
          valueEncoding,
          writable: true,
...
class Tracer {
  ...
  onseq(seq, core) {
    ...
    this.seen.add(seq)
    this.blocks.push(seq)
  }
}

class AutoStore {
  constructor(base, byName, encryption) {
    this.base = base
  ...
  atomize(encryption) {
    const store = new AutoStore(this.base, this.byName, encryption)
  ...
  get(opts, moreOpts) {
    ...
    return this.getViewByName(name).createSession(this.atom, valueEncoding, encryption)
  }
  ...
  getEncryption(name) {
    ...
    return this.encryption.getViewEncryption(name)
  }
  ...
  async getManifestPublicKeys(members) {
    const keys = []

    await Promise.all(
      members.map(({ key }) =>
        collectManifestPublicKey(keys, this.store.get({ key, active: false }))
      )
    )

    return keys

    async function collectManifestPublicKey(collection, core) {
      await core.ready()
      await core.close()

      if (!core.manifest) return null

      for (const signer of core.manifest.signers) {
        collection.push(signer.publicKey)
      }
    }
  }
