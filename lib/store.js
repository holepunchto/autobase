...
class ViewCore {
  ...
  createSession (atom, valueEncoding, encryption) {
    ...
    const s = atom
      ? this._getAtomicBatch(atom, encryption).session({ valueEncoding, writable: true })
...
class AutoStore {
  ...
  async findViewByKey (key, indexers, manifestVersion, entropy) {
    ...
    for (const v of this.byName.values()) {
      ...
      const manifest = this._createManifest(indexerManifests, v.name, null, manifestVersion, entropy, [], manifestData)
