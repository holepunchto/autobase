...
module.exports = class AutoStore {
  ...
  async migrate () {
    ...
    await sys.migrateTo(nextIndexers, sys.indexedLength) // safer to migrate sys first

    for (const ac of this.opened.values()) {
      if (ac === sys) continue

      const view = views[ac.systemIndex]
      await ac.migrateTo(nextIndexers, view ? view.length : 0)
    }
  }
  ...
  async getCore (ac, indexers, length, opts = {}) {
    ...
    const core = this.base.store.get(this._corePreload(indexers, ac.name, prologue, opts.cache))
    if (ac._isSystem()) core.on('append', this._queueFastForward)
    return core
  }
