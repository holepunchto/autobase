...
class ViewCore {
  ...
  createSession (atom, valueEncoding) {
    ...
    const onseq = atom ? this.tracer.onseq.bind(this.tracer) : null

    const s = atom
      ? this._getAtomicBatch(atom).session({ valueEncoding, writable: true, onseq })
      : this.batch.session({ valueEncoding, writable: false, onseq })
