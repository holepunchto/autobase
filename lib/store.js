...
class MemoryStore {
  constructor (viewStore, checkout, atom) {
    this.atom = atom
  ...
  * [Symbol.iterator] () {
  ...
  async flush () {
    ...
    for (const [ac, core] of this.active) {
      ...
      flushing.push(ac.flushWriteBatch(core, treeLength, this.atom))
    }
...
module.exports = class AutoStore {
  ...
  memorySession (checkout) {
    const atom = this.base.store.storage.atom()
    return new MemoryStore(this, checkout, atom)
  }
