...
class MemoryStore {
  constructor (viewStore, checkout, atomizer) {
  ...
  * [Symbol.iterator] () {
  ...
  async flush () {
    ...
    for (const [ac, core] of this.active) {
      ...
      flushing.push(ac.flushWriteBatch(core, treeLength, this.atomizer))
    }
...
module.exports = class AutoStore {
  ...
  memorySession (checkout) {
    const atomizer = this.base.store.storage.atomizer()
    return new MemoryStore(this, checkout, atomizer)
  }

  async migrate (systemLength) {
    const sys = this.getSystemCore()
    const { indexers, views } = await this.base.system.getIndexedInfo(systemLength)
