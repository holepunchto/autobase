...
class FastForwardMigration {
  ...
  async _upgrade () {
    ...
    await sysReady
    await this._migrate(system, sysCore)

    this.key = sysCore.key
  }

  async _migrate (ref, view) {
    ...
    const batch = next.session({ name: 'batch', overwrite: true, checkout: view.length })
    await batch.ready()
  ...
  async _migrateZeroLength (ref) {
    ...
    await this._migrate(ref, { key: next.key, length: 0 })
  }
