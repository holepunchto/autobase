...
module.exports = class Writer extends ReadyResource {
  constructor (base, core, length) {
    ...
    // bg trigger the forced update once (there is bug atm where this doesnt resolve, investigate!)
    this.updating = this.core.update({ wait: true }).then(() => { this.updated = true }, noop)
  }
  ...
  flushed () { // TODO: prop a cleaner way to express this...
    return this.seenLength <= this.length && this.length === this.available && this.length === this.core.length &&
        this.shiftable() === false && !this.core.core.upgrading && this.core.opened && this.updated
  }
