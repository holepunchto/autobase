...
module.exports = class Writer extends ReadyResource {
  ...
  flushCheckpoints (index) {
    ...
    return pending !== undefined && pending !== null ? pending : []
  }

  async _addCheckpoints (checkpoints) {
    for (let i = 0; i < checkpoints.length; i++) {
      const { checkpoint, checkpointer } = checkpoints[i]

      if (checkpointer > 0) continue
      const core = this.base._viewStore.getByIndex(i)

      if (!core) {
        if (!this.pendingCheckpoints[i]) this.pendingCheckpoints[i] = []
        this.pendingCheckpoints[i].push(checkpoint)
        continue
      }
