...
module.exports = class Writer extends ReadyResource {
  ...
  async inflateExistingCheckpoints () {
    ...
    if (this.core.length === 0 || !this._isActiveIndexer || this.nodes.length === 0) {
      return
    }
  ...
  async update () {
    ...
    if (this.digestLength < this.core.length && this._isActiveIndexer) await this._checkDigest()
  ...
  async _loadNextNode () {
    ...
    if (this._isActiveIndexer && checkpoint) {
