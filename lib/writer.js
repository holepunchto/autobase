...
module.exports = class Writer extends ReadyResource {
  ...
  _isDoneDownloading () {
    if (this.length === this.core.length) return true
    if (!this.range) return this.length === this.core.length
    if (!this.range.request.context) console.log(this.range)
    const start = Math.max(this.core.contiguousLength, this.range.request.context ? this.range.request.context.start : this.range.range.end)
    const end = Math.min(this.core.length, this.nodes.length + MAX_PREFETCH)
  ...
  updateActivity () {
    ...
    if (this.seenLength > this.core.length || !this._isDoneDownloading() || this.isBootstrap) {
      // if we have seen a later core, or if we are behind, or if bootstrap
      this.isActive = true
      this.core.setActive(true)
    } else if (this._isDoneDownloading()) {
      // things look steady
      this.isActive = false
