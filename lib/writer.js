...
module.exports = class Writer extends ReadyResource {
  ...
  async _loadCheckpoints (seq, checkpoint) {
    ...
    for (const proof of checkpoint) {
      ...
      if (prev.checkpoint.length <= all.length) continue

      all.push(prev.checkpoint[all.length].checkpoint)
    }
  ...
  async _loadNextNode () {
    ...
    if (this.isIndexer) await this.addCheckpoint(seq, checkpoint)
