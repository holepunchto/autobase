...
module.exports = class Writer extends ReadyResource {
  ...
  append (value, heads, batch, dependencies, version, optimistic) {
    ...
    // can throw
    this._validateNode(node, this.nodes.length)
  ...
  _validateNode (node, seq) {
    if (node.heads.length === 0 && !(b4a.equals(this.core.key, this.base.key) && seq === 0)) {
      throw new Error('Invalid node: empty heads only allowed for genesis')
    }
  }
