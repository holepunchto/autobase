...
module.exports = class Writer extends ReadyResource {
  ...
  async update () {
    ...
    while (this.available - this.length < MAX_PRELOAD) {
      ...
      if (this.length === 0 && !(await this.isInSystem()) && !this.recover) break
  ...
  async _loadNextNode () {
    ...
    // todo: we don't have to null out the values once we figure out how to
    // check removed writers properly, since they are omitted from applyBatch
    const value = this.isRemoved || node.value == null ? null : c.decode(this.base.valueEncoding, node.value)
