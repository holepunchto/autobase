...
module.exports = class Writer extends ReadyResource {
  constructor (base, core, length) {
    ...
    this.core.on('peer-add', this._onpeeradd.bind(this))
    this.core.on('peer-remove', this._onpeerremove.bind(this))
  }
  ...
  async _open () {
    ...
    for (const peer of this.core.peers) this._onpeeradd(peer)
  ...
  _onpeeradd (peer) {
    ...
    this.activePeers.add(hex)
  }

  _onpeerremove (peer) {
    const hex = b4a.toString(peer.remotePublicKey, 'hex')
    this.activePeers.delete(hex)
  }
  ...
  async update () {
    ...
    while (this.available - this.length < MAX_PRELOAD) {
      ...
      if (this.length === 0 && !(await this.isInSystem())) break
