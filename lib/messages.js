...
const V0BootRecord = {
  ...
  decode (state) {
    ...
    return { version: 0, key: null, indexedLength: 0, views, heads, indexed }
  }
}
...
const BootRecord = {
  preencode (state, m) {
    ...
    c.uint.preencode(state, m.indexersUpdated ? 1 : 0)
  },
  encode (state, m) {
    c.uint.encode(state, 1) // version
    c.fixed32.encode(state, m.key)
    c.uint.encode(state, m.indexedLength)
    c.uint.encode(state, m.indexersUpdated ? 1 : 0)
  },
  decode (state) {
    const v = c.uint.decode(state)
    ...
    return {
      version: 1,
      key: c.fixed32.decode(state),
      indexedLength: c.uint.decode(state),
      indexersUpdated: c.uint.decode(state) & 1 ? true : false,
      views: [],
      heads: null,
      indexed: null
    }
