...
const V0BootRecord = {
  ...
  decode (state) {
    ...
    return { version: 0, key: indexed.key, indexedLength: indexed.length, indexersUpdated: false, heads }
  }
}
...
const BootRecord = {
  ...
  encode (state, m) {
    ...
    c.uint.encode(state, (m.indexersUpdated ? 1 : 0) | (m.fastForwarding ? 2 : 0))
  },
  decode (state) {
    const v = c.uint.decode(state)
...
const LinearizerKey = {
  preencode (state, index) {
    IndexEncoder.UINT.preencode(state, 1) // prefix 0 is reserved for future manifest
    IndexEncoder.UINT.preencode(state, index)
  },
  encode (state, index) {
    IndexEncoder.UINT.encode(state, 1) // prefix
