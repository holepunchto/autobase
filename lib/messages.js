...
const BootRecord = {
  ...
  decode (state) {
    ...
    const encryptionLength = v === 1 ? 0 : c.uint.decode(state)
...
const Checkpoint = {
  preencode (state, chk) {
    Checkpointer.preencode(state, chk.system)
    Checkpointer.preencode(state, chk.encryption)
    CheckpointerArray.preencode(state, chk.user)
  },
  encode (state, chk) {
    Checkpointer.encode(state, chk.system)
    Checkpointer.encode(state, chk.encryption)
    CheckpointerArray.encode(state, chk.user)
  },
  decode (state) {
    return {
      system: Checkpointer.decode(state),
      encryption: Checkpointer.decode(state),
      user: CheckpointerArray.decode(state)
    }
  }
}
...
const LinearizerUpdate = {
  ...
  decode (state) {
    ...
    const batch = c.uint.decode(state)
    const systemLength = c.uint.decode(state)
    const encryptionLength = c.uint.decode(state)
    const flags = c.uint.decode(state)
